...
r1_sm, r1_sp, r2, r3, r4 = [
    (0.8259169681505598 + 0j),
...
for r in r_coefficient_list:
    ...
    M1_r_exp = np.array(
        [[r[0], 0, 0, 0], [0, r[1], 0, 0], [0, 0, r[2], 0], [0, 0, 0, r[3]]]
    )
    ...
    T = (
        1
        / np.sqrt(2)
        * np.array([[1j, 0, 1, 0], [0, 1j, 0, 1], [-1j, 0, 1, 0], [0, -1j, 0, 1]])
    )

    T = (
        1
        / np.sqrt(2)
        * np.array([[1j, -1j, 0, 0], [1, 1, 0, 0], [0, 0, 1j, -1j], [0, 0, 1, 1]])
    )
...
for tt in list_tt[0:3]:
    ...
    ticklabels = [
        r"$\left| F2 \right\rangle_1 \left| L \right\rangle_2$",
