...
G = [
    ["g0_KC/2pi", g0_KC * 1000 / (2 * np.pi), "pi", "(F2mf2 -> F'3mf2)"],
...
def Field_in(t, args):
    ...
    t0 = args["t0"]
    return np.sqrt(
        1 / (sigma * np.sqrt(2 * np.pi)) * np.exp(-((t - t0) ** 2) / sigma**2 / 2)
    )
...
# Initialm state
psi0 = tensor(astate[0][0], fock(photondim[0], 0), fock(photondim[0], 0))
...
D = np.linspace(-0.2, 0.2, 160)
for d in D:
    H_D = (
        2 * np.pi * d * (a[0].dag() * a[0] - S[0][1][1])
        + 2 * np.pi * d * (a[1].dag() * a[1] - S[0][0][0])
        + 2 * np.pi * d * (a[0].dag() * a[0] - S[0][3][3])
    )
    H_g = (
        g1_KC * S[0][1][0] * a[0]
        + g2_KC * S[0][2][0] * a[1]
        ...
        sum(
            (
                np.nan_to_num(
                    abs(
                        Field_in(tlist, args)
                        - 0.8 * 1j * np.sqrt(2 * kappa_oc) * output.expect[7]
                    )
                )
                ** 2
            )
    ...
    r_th_sigmaP.append(
        r_theory(d * 2 * np.pi, g1_KC, kappa, kappa_oc, MM_rf, MM_fc, gamma_5P32_5S / 2)
    )
    R_th_sigmaP.append(
        abs(
            r_theory(
                d * 2 * np.pi, g1_KC, kappa, kappa_oc, MM_rf, MM_fc, gamma_5P32_5S / 2
            )
        )
        ** 2
    )
        ...
        sum(
            (
                np.nan_to_num(
                    abs(
                        Field_in(tlist, args)
                        - 0.8 * 1j * np.sqrt(2 * kappa_oc) * output.expect[8]
                    )
                )
                ** 2
            )
    ...
    r_th_sigmaM.append(
        r_theory(d * 2 * np.pi, g2_KC, kappa, kappa_oc, MM_rf, MM_fc, gamma_5P32_5S / 2)
    )
    R_th_sigmaM.append(
        abs(
            r_theory(
                d * 2 * np.pi, g2_KC, kappa, kappa_oc, MM_rf, MM_fc, gamma_5P32_5S / 2
            )
        )
        ** 2
    )
...
H_g = (
    g1_KC * S[0][1][0] * a[0]
)  # + g2_KC * S[0][2][0] * a[1] + g3_KC * S[0][3][2] * a[0]
...
ax[0][0].plot(
    tlist,
    (Field_in(tlist, args) - 0.01 * np.sqrt(2 * kappa_oc) * output.expect[4]) ** 2,
    label="R",
    color="g",
)
ax[0][0].legend()
...
R = (
    np.nan_to_num(
        Field_in(tlist, args) - 0.8 * np.sqrt(2 * kappa_oc) * output.expect[2]
    )
    ** 2
)
