...
def r_empty_cavity(
    mu_rf: float, mu_fc: float, kappa_m1: float, kappa: float, d_c: float
):
...
in_cavity_phase = []
for d in d_list:
    val = r_non_empty_cavity(
        g=g_pi_KC,
        gamma=gamma_5P32_5S * (1 / 10 + 1 / 10 + 3 / 10),
        d_a=d_a_pi + 2 * np.pi * d,
        d_c=d_c_pi + 2 * np.pi * d,
        kappa=kappa,
        kappa_m1=kappa_oc,
        mu_rf=mu_rf,
        mu_fc=mu_fc,
