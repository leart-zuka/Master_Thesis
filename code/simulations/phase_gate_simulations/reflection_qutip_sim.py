...
qced = CavityQEDSystem(photon_dimensions=Photon_dimensions, atom_dimensions=Atom_dimensions)

c_obs = [np.sqrt(2*Kappa)*qced.annihilation_operators["a0"],
...
def simulate(initial_atom_state_index):
    psi0 = qt.tensor(qced.atomic_states[initial_atom_state_index], qt.basis(Photon_dimensions[0], 1))
    H_jc = G_pi_KC * qced.projection_operators[(1,2)] * qced.annihilation_operators["a0"].dag()
    H_drive = np.sqrt(2 * Kappa_oc) * (qced.annihilation_operators["a0"] + qced.annihilation_operators["a0"].dag())
