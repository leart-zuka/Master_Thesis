...
for d in d_list:
    ...
    in_cavity_phase.append(cmath.phase(val)/np.pi)
...
for d in d_list:
    ...
    not_in_cavity_phase.append(cmath.phase(val)/np.pi)
...
ax[0,0].plot(d_list,in_cavity, 'b') 
...
ax[0,1].plot(d_list,not_in_cavity, 'k') 
ax[0,1].axvline(x=0.0)
ax[0,1].add_artist(AnnotationBbox(OffsetImage(image_pi_down,zoom=0.34), (0.42,0.35),frameon=True))
ax[0,1].set_xlabel('detuning in (GHz)')
ax[0,1].set_ylabel('reflectivity')

ax[1,0].plot(d_list, in_cavity_phase, "b")
ax[1,0].axvline(x=0.0)
ax[1,0].set_xlabel("destuning in (GHz)")
ax[1,0].set_ylabel("phase shift in (rad)")

ax[1,1].plot(d_list, not_in_cavity_phase, "b")
ax[1,1].axvline(x=0.0)
ax[1,1].set_xlabel("destuning in (GHz)")
