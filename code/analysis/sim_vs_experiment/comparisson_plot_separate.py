...
def plot_bw_ax(ax, label=None):
    """Draw the Brewster window scan onto an existing Axes."""
    ax.plot(
        bw_trans_th,
        bw_overlap_th,
        "-",
    ...
    ax.set_xlabel(r"$V$-mode transmission")
...
def plot_pn_ax(ax, label=None):
    """Draw the photon number scan onto an existing Axes."""
    ax.plot(
        pn_number_th,
        pn_overlap_th,
        "-",
...
plot_bw_ax(ax1, label="(a)")
plot_pn_ax(ax2, label="(b)")
fig.tight_layout(w_pad=3)
plt.savefig("sim_comparison_plots.svg", bbox_inches="tight")
plt.savefig("sim_comparison_plots.png", bbox_inches="tight", dpi=250)
plt.close()

# ── SUBFIGURE (a): Brewster window scan ──
fig_a, ax_a = plt.subplots(figsize=(5, 4))
plot_bw_ax(ax_a)
fig_a.tight_layout()
plt.savefig("sim_comparison_bw_scan.svg", bbox_inches="tight")
plt.savefig("sim_comparison_bw_scan.png", bbox_inches="tight", dpi=250)
plt.close()

# ── SUBFIGURE (b): Photon number scan ──
fig_b, ax_b = plt.subplots(figsize=(5, 4))
plot_pn_ax(ax_b)
fig_b.tight_layout()
plt.savefig("sim_comparison_pn_scan.svg", bbox_inches="tight")
plt.savefig("sim_comparison_pn_scan.png", bbox_inches="tight", dpi=250)
plt.close()

print("Done — saved combined + two individual figures.")
