...
class AtomAnalysis:
    ...
    def dataEv_MWspectrumTrapScan(self, path: str, filename : str, filetype : str = 'h5', trapPower : List[float] = [130]):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts = 1600, no = 100)
        ...
        for i in range(len(trapPower)):
            ...
            for pfs in tqdm(dataDic[self.syncFast2][1][:-1], file=sys.stdout):
...
if __name__ == '__main__':
    ...
    measurements = "24_09_25_MW_Spectroscopy_Trap_7"
    filetype : str = ".h5"
    ...
    MWfreqCenter = MWfrequencies[int(len(MWfrequencies) / 2)]
    MWfreqStart, MWfreqEnd = MWfrequencies[0], MWfrequencies[-1]
    MWfreqStepsPerRamp = len(MWfrequencies)
    MWfreqStep = (MWfreqEnd - MWfreqStart) / MWfreqStepsPerRamp
    ...
    for i in range(len(trapPws)):
        ...
        if fit_peaks:
            ...
            threshold = 0.05
            window_size_3 = int(MWfreqStepsPerRamp / 3 - 1)
            window_size_5 = int(MWfreqStepsPerRamp / 5 - 1)
            window_size_7 = int(MWfreqStepsPerRamp / 7 - 1)
            maxima_indices, maxima_values = analysis.find_local_maxima_in_range(np.array(SDmean[i]), window_size_3, threshold)
            ...
            # # fitting the seven different peaks
            modMWspec = [None] * len(maxima_values)
            res_MWspec = [None] * len(maxima_values)

            c0, c1, c2, l0, l1, l2, r0, r1, r2 = 0, 0, 0, 0, 0, 0, 0, 0, 0  # center0, center1, center2
            ...
            if n_peaks == 5:
                ...
                l_index = (maxima_indices - 15/MWfreqStep).astype(int)
                r_index = (maxima_indices + 15/MWfreqStep).astype(int)
        ...
        if fit_peaks:
            for j in range(len(modMWspec)):
                ...
                Delta[i] = np.round(res_MWspec[2].best_values['foff'], 1) - np.round(res_MWspec[0].best_values['foff'],
                                                                                     1)
                Delta_stderr[i] = np.round(res_MWspec[2].params['foff'].stderr, 2) + np.round(
                ...
                fig.suptitle(measurements + " - trap Power = %.1f mW"%(trapPws[i]) + '\n' +
                    ...
                    np.round(2 * res_MWspec[1].best_values['k1'], 2)) + 'kHz\n'
                                                                        'foffs3 = ' + str(
                    np.round(res_MWspec[2].best_values['foff'], 1)) + 'kHz' + '       FWHM3 = ' + str(
                    np.round(2 * res_MWspec[2].best_values['k1'], 2)) + 'kHz\n'
                                                                        r'$\Delta$(1-3) = ' + str(
                    Delta[i]) + ' pm ' + str(Delta_stderr[i]) + ' kHz', fontsize=afs)
