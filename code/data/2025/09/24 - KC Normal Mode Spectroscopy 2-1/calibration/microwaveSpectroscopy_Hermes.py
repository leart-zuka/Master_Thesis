...
class AtomAnalysis:
    ...
    def dataEv_atomPhotonCoincidences(self, path: str, file_list: List[str], filetype: str = '.h5', position: int = 0,
                                      heralded: bool = True):
    ...
    def dataEv_writeAtom(self, path: str, file_list: List[str], filetype: str = 'h5', position: int = 0):
        ...
        photonkcEff, photonkcEfferr = (sum(photonkcV) + sum(photonkcH)) / fstot, np.sqrt(
        ...
        pF2, pF2err = (kcHF2 + kcVF2) / (kcHF1 + kcHF2 + kcVF1 + kcVF2), np.sqrt(kcHF2 + kcVF2) / (
                    kcHF1 + kcHF2 + kcVF1 + kcVF2)
        pF1, pF1err = (kcHF1 + kcVF1) / (kcHF1 + kcHF2 + kcVF1 + kcVF2), np.sqrt(kcHF1 + kcVF1) / (
                    kcHF1 + kcHF2 + kcVF1 + kcVF2)
...
def analyse(path: str, measurements:str, filetype:str) -> Tuple[float, float]:
    # ------ Begin Analysis ------
    ...
    if fit_peaks:
        ...
        maxima_indices, maxima_values = analysis.find_local_maxima_in_range(np.array(SDmean), window_size_3, threshold)
        ...
        if n_peaks == 3:
            ...
            for j in range(len(modMWspec)):
                ...
                (modMWspec[j], res_MWspec[j]) = analysis.dataFit_lorenzian(MWfrequencies[l_index[j]:r_index[j]],
                                                                           SDmean[l_index[j]:r_index[j]],
                                                                           err_SDmean[l_index[j]:r_index[j]])

            freqFitPlot = [np.linspace(MWfrequencies[l_index[j]], MWfrequencies[r_index[j]], 1000) for j in
                           range(len(modMWspec))]

    # ------ Plotting ------
    afs = 16  # axis font size
    txt = measurements
    fig = plt.figure(txt, figsize=(8, 4))

    ax1 = fig.add_subplot(1, 1, 1)

    plt.errorbar(MWfrequencies, SDmean, yerr=err_SDmean, marker="o", linestyle="None", color="black",
                 capsize=3)
    string = measurements + '\n'
    if fit_peaks:
        for j in range(len(modMWspec)):
            ...
            Delta_stderr = np.round(res_MWspec[2].params['foff'].stderr, 2) + np.round(
            ...
            fig.suptitle(measurements + '\n' +
                ...
                np.round(2 * res_MWspec[2].best_values['k1'], 2)) + 'kHz\n'
                                                                    r'$\Delta$(1-3) = ' + str(
                Delta) + ' pm ' + str(Delta_stderr) + ' kHz', fontsize=afs)
    ...
    plt.clf()
    if fit_peaks:
        return Delta, Delta_stderr

def base_function(x):
    '''
    base function determined on the 26.02.2025
    :param x: angle in grad
    :return: splitting at specific angle
    '''
    return -2.2618*x**2 + 518.97*x - 29449
...
if __name__ == '__main__':
    ...
    print("Started socket on local machine")
    print("Waiting for data from Hermes...")

    # Settings for file
    filetype: str = ".h5"

    # Settings for output
    # Splitting needs to be in range 224-226
    desired_splitting = 219
    angle_step = 0.5 # only use in worst case
    splitting_lower_bound = 219.5
    splitting_upper_bound = 218.5
    data_x: List[float] = []
    data_y: List[float] = []
    a = -2.2618
    b = 518.97
    c = -29449
    c_for_solving = c-desired_splitting
    ...
    try:
        while True:
            ...
            print("Received data from Hermes")
            print(f"Data: {message}")
            file_name = message["filename"]
            num_of_peaks = message["num_of_peaks"]
            current_angle = message["current_angle"]
            ...
            try:
                ...
                # weird ahh file path settings
                path = "\\".join(file_name.split("\\")[:-1])
                measurement = file_name.split("\\")[-1]

                print(path, measurement)
                print(os.path.exists(path + "\\" + measurement + filetype))
                time.sleep(5)


                # ------ Begin Analysis ------
                delta, delta_stderr = analyse(path=path, measurements=measurement, filetype=filetype)

                # attach warning message
                response["warning"] = f"Didn't linearize the field, had to stop, last splitting was: {delta}"

                # if we do get some results from our analysis though, change the response accordingly
                if (delta is not None) or (delta_stderr is not None):
                    if (delta >= splitting_lower_bound-delta_stderr) and (delta <= splitting_upper_bound + delta_stderr):
                        response = {
                            "retry": False,
                            "new_angle": current_angle
                        }
                    else:
                        data_x.append(current_angle)
                        data_y.append(delta)

                        try:
                            if len(data_x) == 1:
                                new_angle_for_fitting = (-1 * b + np.sqrt(b ** 2 - 4 * a * c_for_solving)) / (2 * a)
                                preliminary_x = [data_x[0], new_angle_for_fitting]
                                preliminary_y = [data_y[0], desired_splitting]
                                x_data_for_fitting = np.linspace(min(preliminary_x), max(preliminary_x), 2)
                                y_data_for_fitting = np.linspace(min(preliminary_y), max(preliminary_y), 2)
                                popt, pcov = curve_fit(linear_function, x_data_for_fitting, y_data_for_fitting,p0=[20, 100])
                                new_angle = (desired_splitting - popt[1]) / popt[0]
                            elif len(data_x) == 2:
                                x_data_for_fitting = np.linspace(min(data_x), max(data_x), 2)
                                y_data_for_fitting = np.linspace(min(data_y), max(data_y), 2)
                                popt, pcov = curve_fit(linear_function, x_data_for_fitting, y_data_for_fitting,p0=[20, 100])
                                new_angle = (desired_splitting - popt[1]) / popt[0]
                            elif len(data_x) > 2:
                                x_data_for_fitting = np.linspace(min(data_x), max(data_x), 10)
                                y_data_for_fitting = np.linspace(min(data_y), max(data_y), 10)
                                popt, pcov = curve_fit(quadratic_function, x_data_for_fitting, y_data_for_fitting)
                                new_angle = (-1 * popt[1] + np.sqrt(popt[1] ** 2 - 4 * popt[0] * (popt[2]-desired_splitting))) / (2 * popt[0])
                        except Exception as e:
                            print("Wasn't able to get new angle via fitting, just gonna use the fixed angle step")
                            if delta <= splitting_lower_bound - delta_stderr:
                                new_angle = current_angle + angle_step
                            elif delta >= splitting_upper_bound + delta_stderr:
                                new_angle = current_angle - angle_step
                ...
                # attach warning message
                response["warning"] = f"Didn't linearize the field, analysis failed"
                print(f"Following error occurred during analysis: {e}")

            try:
                socket.send_json(response)
            except Exception as e:
                print(f"Wasn't able to send message to hermes due to: {e}")

    except KeyboardInterrupt:
        print("Calibration was stopped")
    finally:
        socket.close()
        context.term()
