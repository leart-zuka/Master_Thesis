...
class AtomAnalysis:
    ...
    def dataEval_noramlModeSpectroscopy(
        ...
        p0 = [
            ...
            70,  # g (coupling strength, MHz)
            60,  # kappa (total cavity decay rate, MHz)
            60 * 0.85,  # kappa_oc (outcoupling, ~85% of total kappa)
            0.882,  # MM_rf (close to ideal)
            0.882,  # MM_fc (80% coupling in)
            3,  # gamma (free space decay rate, MHz)
            0.01,  # offset (background level)
            0.0,  # a (slope term for detuning-dependent broadening)
        ]

        bounds = (
            [-50, 0.01, 50, 55.4, 55.4 * 0.8, 0.95, 0.7, 2.9, 0.01, -0.00001],
            [-20, 0.3, 90, 60, 51, 1, 0.9, 3.3, 0.03, 0.00001],
        )
        bounds = (
            [
                -np.inf,
