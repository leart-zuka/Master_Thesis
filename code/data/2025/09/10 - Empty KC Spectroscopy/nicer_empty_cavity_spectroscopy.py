...
if splitting_flag == True:
    ...
    spec_fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(7.0, 4.8), sharex=True)
    spec_fig.subplots_adjust(hspace=0.08)
...
ax1.text(
    0.02,
    0.93,
    r"\textbf{(a)}" if plt.rcParams["text.usetex"] else r"$\mathbf{(a)}$",
    transform=ax1.transAxes,
    fontsize=12,
    va="top",
)
# Add a single legend entry for data vs fit (using proxy artists)
from matplotlib.lines import Line2D

_leg_handles = [
    Line2D([], [], color="0.55", linewidth=0.8, label="Data"),
    Line2D([], [], color="#C0392B", linewidth=1.2, label="Lorentzian fit"),
]
ax1.legend(handles=_leg_handles, loc="upper right", handlelength=1.5)
...
ax2.text(
    0.02,
    0.93,
    r"\textbf{(b)}" if plt.rcParams["text.usetex"] else r"$\mathbf{(b)}$",
...
# Remove x tick labels from top panel since axes are shared
ax1.tick_params(labelbottom=False)
spec_fig.align_ylabels([ax1, ax2])
# plt.savefig("Normal_mode_spectroscopy.svg", bbox_inches="tight")
plt.savefig("empty_mode_spectroscopy.svg", bbox_inches="tight")
plt.savefig("empty_mode_spectroscopy.pdf", bbox_inches="tight")
plt.savefig("empty_mode_spectroscopy.png", dpi=300, bbox_inches="tight")
