...
class Analyzer:
    ...
    def reflection_analysis(
        ...
        write_gate = [
            optical_pumping_gate[1] + pulse_delay_SD_before,
            optical_pumping_gate[1] + pulse_delay_SD_before+pulse_duration_SD_before,
        ...
        for i in track(range(length)):
            ...
            start_sd_before = t0 + write_gate[0]
            end_sd_before = t0 + write_gate[1]
            start = t0 + write_gate[2]
            end = t0 + write_gate[3]
            start_sd_after = t0 + write_gate[4]
            end_sd_after = t0 + write_gate[5]

            start_4_sd_before, end_4_sd_before, start_4, end_4, start_4_sd_after, end_4_sd_after = np.searchsorted(
                data_arr[self.kc_h],
                [start_sd_before,end_sd_before,start, end, start_sd_after, end_sd_after],
            )

            start_7_sd_before, end_7_sd_before, start_7, end_7, start_7_sd_after, end_7_sd_after = np.searchsorted(
                data_arr[self.kc_v],
                [start_sd_before,end_sd_before,start, end, start_sd_after, end_sd_after],
            )
            n4_sd_before = end_4_sd_before - start_4_sd_before
            n7_sd_before = end_7_sd_before - start_7_sd_before

            n4 = end_4 - start_4
            n7 = end_7 - start_7

            n4_sd_after = end_4_sd_after - start_4_sd_after
            n7_sd_after = end_7_sd_after - start_7_sd_after

            if (n7_sd_before + n4_sd_before) == 0:
                if (n4 + n7 > 0):
                    if post_select_sd:
                        if "atom_0" in file_name:
                            if (n4_sd_after + n7_sd_after) == 0:
...
if __name__ == "__main__":
    ...
    file = "03_04_26_AP_Gate_CNOT_RL_Basis_0_2_photons_atom_1_test_1"
    ...
    ParamDictReflection_trap_off: ReflectionGateT = {
        ...
        "pulse_delay": 46.6e-6,  # Trap on
        "pulse_duration": 1.2e-6,
        # "pulse_delay_SD": 2e-6, # Trap off
        "pulse_delay_SD_after": 1.1e-6,  # Trap on
        "pulse_duration_SD_after": 7e-6,
        "pulse_delay_SD_before": 38.5e-6,  # Trap on
        "pulse_duration_SD_before": 7.1e-6,
    ...
    cnot_files_trap_on = [  # only use half len/2
        "03_04_26_AP_Gate_CNOT_RL_Basis_0_2_photons_1_3",
        "03_04_26_AP_Gate_CNOT_RL_Basis_0_2_photons_1_4",
        "03_04_26_AP_Gate_CNOT_RL_Basis_0_2_photons_1_1",
        "03_04_26_AP_Gate_CNOT_RL_Basis_0_2_photons_1_2",
    ]
    ...
    overlap = (
        cnot_gate[0][0] + cnot_gate[1][1] + cnot_gate[2][3] + cnot_gate[3][2]
    ) / 4

    # Error propagation: sum of independent errors in quadrature, divided by 4
    overlap_err = (
        np.sqrt(
            cnot_err[0][0] ** 2
            + cnot_err[1][1] ** 2
    ...
    plot_gate_3d(
        ...
        file_name="CNOT @ mean photon of 0_2 RL",
