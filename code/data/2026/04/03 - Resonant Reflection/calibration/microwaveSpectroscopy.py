...
if __name__ == "__main__":
    ...
    for i in range(1, 2):
        ...
        # ------ Data evaluation ------
        (MWfrequencies, SDmean, err_SDmean) = analysis.dataEv_MWspectrum(
            path, measurements, filetype
        )
        ...
        maxima_indices, maxima_values = analysis.find_local_maxima_in_range(
            np.array(SDmean), window_size_5, threshold
        )

        print("Indices of local maxima above threshold:", maxima_indices)
        print("Local maxima values above threshold:", maxima_values)
        print(
            "Frequencies of local maxima values above threshold:",
            [MWfrequencies[i] for i in maxima_indices],
        )
        ...
        if fit_peaks:
            ...
            if n_peaks == 5:
                c = np.array(
                    [MWfrequencies[maxima_index] for maxima_index in maxima_indices]
                )
                l = c - 15
                r = c + 15
                ...
                r[r > (MWfreqEnd - MWfreqCenter)] = (
                    MWfreqEnd - MWfreqCenter - MWfreqStep
                )
                ...
                for i in range(len(modMWspec)):
                    ...
                    (modMWspec[i], res_MWspec[i]) = analysis.dataFit_lorenzian(
                        MWfrequencies[l_index[i] : r_index[i]],
                ...
                freqFitPlot = [
                    np.linspace(
                        MWfrequencies[l_index[i]], MWfrequencies[r_index[i]], 1000
                    )
                    for i in range(len(modMWspec))
                ]
                ...
                l1_index, r1_index = (
                    int((l1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep),
                    int((r1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep),
                )
                print(l1_index, r1_index)

                modMWspec, res_MWspec, c = [0], [0], [c1]
                (modMWspec[0], res_MWspec[0]) = analysis.dataFit_lorenzian(
                    MWfrequencies[l1_index:r1_index],
                    SDmean[l1_index:r1_index],
                    err_SDmean[l1_index:r1_index],
                )

                freqFitPlot = [
                    np.linspace(MWfrequencies[l1_index], MWfrequencies[r1_index], 1000)
                ]
        ...
        df = pd.DataFrame(
            {"MWfrequencies": MWfrequencies, "SDmean": SDmean, "err_SDmean": err_SDmean}
        )
        df.to_csv(path + "/" + measurements + ".csv", index=False)
        ...
        if fit_peaks:
            ...
            for i in range(len(modMWspec)):
                ax1.plot(
                    freqFitPlot[i],
                    modMWspec[i].eval(freq=freqFitPlot[i], params=res_MWspec[i].params),
                    "r",
                )
                ax1.text(
                    c[i],
                    np.round(res_MWspec[i].best_values["amp1"], 1),
                    np.round(res_MWspec[i].best_values["foff"], 1),
                    fontsize=afs,
                )
                string += (
                    "foff "
                    + str(i + 1)
                    + " = "
                    + str(np.round(res_MWspec[i].best_values["foff"], 1))
                    + "kHz"
                    + "       FWHM "
                    + str(i + 1)
                    + " = "
                    + str(np.round(2 * res_MWspec[i].best_values["k1"], 2))
                    + "kHz\n"
                )
