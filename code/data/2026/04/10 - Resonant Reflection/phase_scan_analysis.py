...
from helper.analysis_types import NormalModeSpectroscopyT, ReflectionGateT, Bell_State_Phase_ScanT
...
class Analyzer:
    ...
    def bell_state_phase_scan_analysis(
        self,
        file_name: str,
        parameters: Bell_State_Phase_ScanT,
        path: str | Path | None = None,
        file_type: str = ".h5",
        plot_histogram: bool = False,
    ):
        ...
        for i in track(range(length)):
            ...
            start_sd = t0 + write_gate[0]
            end_sd = t0 + write_gate[1]

            start_4_sd, end_4_sd = np.searchsorted(
                data_arr[self.kc_h],
                [start_sd, end_sd]
            )

            start_7_sd, end_7_sd = np.searchsorted(
                data_arr[self.kc_v],
                [start_sd, end_sd],
            )
...
if __name__ == "__main__":
    ...
    file = "10_04_26_Bell_state_mw_phase_scan_1"

    ParamDictBellStatePhaseScan: Bell_State_Phase_ScanT = {
        "trigger_delay": 3.15e-6,
        "cooling_duration": 400e-6,
        "optical_pumping_duration": 340e-6,
        "pulse_delay_SD": 40e-6,  # Trap on
        "pulse_duration_SD": 8e-6,
        "sequence_duration": 1.1e-3,
    }

    (
        P_F2,P_F1
    ) = analyzer.bell_state_phase_scan_analysis(
        file_name=file,
        path="./",
        parameters=ParamDictBellStatePhaseScan,
        plot_histogram=True,
    )

    print(f"P(F=1)={P_F1*100:.2f}%")
    print(f"P(F=2)={P_F2*100:.2f}%")
