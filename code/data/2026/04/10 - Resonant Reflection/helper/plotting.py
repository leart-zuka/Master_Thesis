...
def plot_gate_3d(
    ...
    fidelity = sum(matrix[i, j] for i, j in fidelity_indices) / 3
    fidelity_err = np.sqrt(sum(err_matrix[i, j] ** 2 for i, j in fidelity_indices)) / 3
    ...
    # ---- Error bars ----
    errors = err_matrix.ravel()
    heights = matrix.ravel()
    cap_size = 0.08
    for xi, yi, h, err in zip(x, y, heights, errors):
        if err > 0:
            cx = xi + width / 2
            cy = yi + depth / 2
            # Vertical line
