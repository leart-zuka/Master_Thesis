...
class DataLoading:
    ...
    # ------ Data loading ------
    def data_loading(self, path: str, fileName, atom: int):
        """
        INPUTS
...
class AtomAnalysis:
    ...
    def dataEv_postSelection(self, path: str, filename: str, filetype: str = '.h5', kcCounts: int = 2000, no = 10):
        ...
        for i in tqdm(atomList, file=sys.stdout):
            ...
            # (dataDic) = self.load.data_loading(path, filedata, i)
        ...
        if self.psSave == True:
            ...
            print('Measurement start/end: %.1f/%.1f' % (atomInHisto[0],atomOutHisto[-1]))
    ...
    def dataEv_writeAtom(self, path: str, file_list: List[str], filetype: str = 'h5', position: int = 0):
        ...
        photonlcH, photonlcV, photonkcH, photonkcV = [], [], [], []
        photonSD = [[], []]  # n of photons in SD gate for each trial
        photonNoise = [[], []]  # n of photons in noise gate for each trial
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts= 30, no = 30)

            a_file = open(file_postSelected, "rb")
            # atomDic = pickle.load(a_file)
            # atomDic = {0: [14.633597090607509, 112.19760588057397], 1: [4.202617019051104, 9.332265541364904], 2: [7.81491686985828, 8.432598266168498], 3: [7.884058201714652, 8.984447863971582], 4: [14.934185050864471, 22.762603915471118]}
            atomDic0 = pickle.load(a_file)
            # print(atomDic0)
            atomDic = {k: atomDic0[k] for k in list(atomDic0)[:]}
            ...
            if analysis_flag == 'local':
                ...
                OPdur = 50e-6
                com_time = 2.56e-6
                pulse_duration = 150e-9
                MWpulse_delay = 10.4e-6
                MWpulse1 = 29.6e-6 #100kHz Lamor frequency
                MWpulse2 = 53.4e-6/2 #100kHz Lamor freqency
                ...
                SDgate = [0.7828e-3,#+10e-6,
                          0.7901e-3#+10e-6
                          ]
                noisegate = [trigger_delay + OPdur + trapOffdelay+2e-6, trigger_delay + OPdur + trapOffdelay + noiseDur + 2e-6]

                # writegate = [0.06195e-3,0.06205e-3] #old
                writegate = []  # old

                writegateList = []
                for i in range(22):
                    writegateList.append([0.78083e-3, 0.78083e-3 + (i+1)*0.05e-6])
                    writegate=writegate+[0.78083e-3, 0.78083e-3 + (i+1)*0.05e-6]
                # noisegate = [0.0536e-3,0.0536e-3+noiseDur]
                gates = writegate + SDgate
                # writegateList.append([0.7808e-3, 0.7819e-3])
                # print(writegateList)

            elif analysis_flag == 'lmu':
                seqDur = 0.255e-3

                trigger_delay = 3.15e-6
                OPdur = 50e-6
                com_time = 6.4e-6 #3.73e-6
                com_time_photon = 6.59e-6 + 2.245e-6 # additional delay of 2.23us due to considering trigger from LMU
                pulse_duration = 250e-9
                photonDur = 85e-9
                MWpulse_delay = 6e-6#2e-6
                MWpulse1 = 29.6e-6 #100kHz Lamor frequency
                MWpulse2 = 53.4e-6/2 #100kHz Lamor freqency
                # MWpulse1 = 13.8e-6 #300kHz Lamor frequency
                # MWpulse2 = 25e-6/2 #300kHz lamor frequency
                SDdur = 8e-6
                coinc_delay = 4.8e-6 #1.5e-6
                trapOffdelay = 0.5e-6 + 2.23e-6
                noiseDur = 5e-6


                opgate = [trigger_delay, trigger_delay + OPdur]
                writegate = [opgate[1] + com_time, opgate[1] + com_time + pulse_duration]
                photongate = [opgate[1] + com_time_photon, opgate[1] + com_time_photon + photonDur]
                writegate = photongate
                coincgate = [writegate[1] + coinc_delay, writegate[1] + coinc_delay + pulse_duration ]
                SDgate = [writegate[1]+MWpulse_delay+MWpulse1+MWpulse2, writegate[1]+MWpulse_delay+MWpulse1+MWpulse2+SDdur]
                noisegate = [trigger_delay + OPdur + trapOffdelay, trigger_delay + OPdur + trapOffdelay + noiseDur]

                gates = writegate + SDgate + coincgate + opgate + noisegate
                # gates = noisegate
                ...
                pulseDur = 250e-9
                SDDur = 7.5e-6
                ext_trigger_delay = 349.16e-6


                opgate = [cooldur, cooldur + OPdur]
                writegate = [opgate[1]-ext_trigger_delay, opgate[1] + pulseDur-ext_trigger_delay]
                SDgate = [0.0873e-3, 0.0873e-3 + SDDur]
                coincgate = [404e-6-ext_trigger_delay, 405e-6-ext_trigger_delay]
            ...
            pF1err_list = []
            pF2err_list = []
            for writegate in writegateList:
                fstot = 0
                sdTrigtot = 0
                tlcH, tlcV, tkcH, tkcV = [], [], [], []
                photonlcH, photonlcV, photonkcH, photonkcV = [], [], [], []
                ...
                for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                    fs = dataDic[trig][1][
                        np.searchsorted(dataDic[trig][1], fsr) + position]  # + position*1e-6
                    # kcH herald
                    sf = fs + fsdelay[self.kcH]
                ...
                noisetot = sum(photonNoise[0])
                noiseTime = noiseDur*fstot
                noiseRate = noisetot/(noiseTime)
                pNoise = noiseRate*photonDur
                noiseBg = pNoise * fstot
                if noiseBg > 0:
                    SNR = (sum(photonkcH)-noiseBg)/noiseBg

                print("============================================")
                print("             Herald analysis")
                print("============================================")
                print("Number of Herald photons = %.0f" % sum(photonkcH))
                print("Efficiency kcH = %.7f(%.5f)" % (effkcH, effkcH_err))
                print("Efficiency kcV = %.7f(%.5f)" % (effkcV, effkcV_err))
                print("Efficiency herald total = %.7f(%.5f)" % (photonkcEff, photonkcEfferr))
                # if analysis_flag == "local":
                #     photonlcEff, photonlcEfferr = (sum(photonlcV) + sum(photonlcH)) / fstot, np.sqrt(sum(photonlcV) + sum(photonlcH)) / fstot
                #     print("============================================")
                #     print("             Reflection analysis")
                #     print("============================================")
                #     print("Number of reflected photons = %.0f" % (sum(photonlcH)+sum(photonlcV)))
                #     print("Efficiency reflection total = %.7f(%.5f)" % (photonlcEff, photonlcEfferr))
                # print("============================================")
                # print("             Noise analysis")
                # print("============================================")
                # print("Number of fs trigger = %.0f" % fstot)
                # print("Number of noise clicks = %.0f" % noisetot)
                # print("Sum noise windows = %.2f s" % noiseTime)
                # print("Noise rate = %.4f Hz" % noiseRate)
                # print("P(Noise photon in Herald gate) = %.9f" % pNoise)
                # print("Estimated Noise background in Herald window = %.3f" % noiseBg)
                # if noiseBg > 0:
                #     print("SNR = %.2f" % SNR)
                # else:
                #     print("SNR = inf")
                ...
                print("Probability F1 = %.3f(%.4f)" % (pF1, pF1err))
                print("Probability F2 = %.3f(%.4f)" % (pF2, pF2err))
                pF1_list.append(pF1)
                pF2_list.append(pF2)
                pF1err_list.append(pF1err)
                pF2err_list.append(pF2err)
        plt.figure(filename)
        plt.errorbar(np.linspace(0,11,22),pF1_list,pF1err_list,label="F1")
        plt.errorbar(np.linspace(0,11,22),pF2_list,pF2err_list,label="F2")
        plt.legend()
        plt.xlabel("Pulse gate")
        ...
        def emg(x, A, mu, sigma, lambd):
            """Exponentially Modified Gaussian (convolution of Gaussian and exponential).

            Parameters:
            A     : Amplitude (normalization)
            mu    : Mean of the Gaussian
            sigma : Std dev of the Gaussian
            lambd : Decay rate of the exponential (1/tau)
            """
            conv = (lambd / 2) * np.exp((lambd / 2) * (2 * mu + lambd * sigma ** 2 - 2 * x))
            conv *= erfc((mu + lambd * sigma ** 2 - x) / (np.sqrt(2) * sigma))
            return A * conv

        bin_width = 3.33e-9*3
        historange = [writegate[0], writegate[1]]
        histoBinNum = int(((historange[1] - historange[0])) / bin_width)
        histotime = (np.linspace(historange[0], historange[1], histoBinNum) - historange[0]) * 1e9  # ns
        model_time=np.linspace(0,(historange[1]-historange[0])*1e9,1000)
        histogram_kc = np.histogram(tkcH + tkcV, range=historange, bins=histoBinNum)[0]


        # # 2. Fit the EMG to the histogram
        p0 = [0.01, 2500, 800]  # Initial guesses
        bounds = ([0, -np.inf, 1e-3], [np.inf, np.inf, np.inf])  # Avoid zero/negative sigma/lambda
        popt, pcov = curve_fit(gaussian, histotime, histogram_kc/fstot, p0=p0, bounds=bounds)

        effkcText=''
        plt.rcParams.update({'font.size': 15})
        spttl = filename + " - Photon"
        plt.close(spttl)
        if analysis_flag == 'local':
            phfig = plt.figure(spttl, figsize=[10, 10])
        else:
            phfig = plt.figure(spttl, figsize=[10, 7])

        phfig.suptitle(filename + "\n Herald photon histogram "
                                  "\n bin width= %.1f ns "
                                  "\n Number of Herald photons = %.0f"
                                  "\n Efficiency kcH= %.7f(%.5f)"
                                  "\n Amplitude: %.1f  "
                                  "\n Center: %.1f ns "
                                  "\n Std. $\sigma$: %.1f" % (bin_width * 1e9,sum(photonkcH),effkcH, effkcH_err, popt[0], popt[1], popt[2]))
        ax = phfig.add_subplot(2, 1, 1)
        ax.bar(histotime, histogram_kc / fstot, width=bin_width * 1e9, color="red", label="Herald photon data")
        ax.plot(model_time, gaussian(model_time, *popt), label='Gaussian fit', color='blue', linewidth = 2, linestyle = '--')
        # ax.plot(model_time, emg(model_time, 0.0297, 36, 8.9,0.038), label='lmu photon simulated', color='green', linewidth = 3, linestyle = '--')

        if analysis_flag == 'local':
            histogram_lc = np.histogram(tlcH + tlcV, range=historange, bins=histoBinNum)[0]

            ax2 = phfig.add_subplot(2, 1, 2)
            ax2.bar(histotime, histogram_lc / fstot, width=bin_width * 1e9, color="orange", label="Reflection photon data")
            ax2.legend()
            ax2.set_xlabel("time (ns)")
            ax2.set_ylabel("counts/trials")
...
if __name__ == '__main__':
    ...
    # name = "24_07_25_lmu-cavX_heralding_6"
    file_list = [
                # "08_04_26_AP_Gate_02_photons_RL_basis_sending_L_atom_1_5mus_1",
                # "08_04_26_AP_Gate_02_photons_RL_basis_sending_L_atom_1_10mus_1",
                # "08_04_26_AP_Gate_02_photons_RL_basis_sending_L_atom_1_10mus_SD_1mus_later_1"
                "08_04_26_AP_Gate_02_photons_RL_basis_sending_L_atom_1_1mus_SD_1mus_later_1"
                 ] #,"24_07_25_lmu-cavX_heralding_7","24_07_25_lmu-cavX_heralding_8","24_07_25_lmu-cavX_heralding_9"]
    ...
    trig = 0
    analysis.dataEv_writeAtom(path, file_list, filetype, position=trig)
    ...
    #         counts[base] = analysis.dataEv_writeAtom(path, file_list, filetype, position=trig)
    #         print(base)
