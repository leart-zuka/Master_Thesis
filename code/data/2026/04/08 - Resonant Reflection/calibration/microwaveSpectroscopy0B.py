...
class AtomAnalysis:
    ...
    def dataEv_postSelection(self, path: str, filename: str, filetype: str = '.h5', kcCounts: int = 1500, no=1):
        ...
        for i in tqdm(atomList, file=sys.stdout):
            ...
            wt_kc = 0.5 * kcCounts  # wt = witness threshold
            wt_lc = -1  # wt = witness threshold
            twot = 3 * kcCounts  # twot = two atom threshold
            inAtom = False
            atomIn_index = 0
            atomOut_index = 0
    ...
    def dataEv_MWRabiRot(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        SDstart = [coolingDur + OPdur +delay + m for m in timeIntervals]
    ...
    def dataEv_MWspectrum(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts = 1500, no = 100)

        a_file = open(file_postSelected, "rb")
        atomDic = pickle.load(a_file)
        # ------ Definition of parameter values ------
        MWfreqCenter = 0  # in kHz
        MWfreqSpan = 50  # in kHz
        # MWfreqSpan = 500  # in kHz
        ...
        MWfreqStepsPerRamp = 100  # number of microwave  steps per ramp
        # MWfreqStepsPerRamp = 1000  # number of microwave  steps per ramp
        ...
        SDstart = seqDur - SDdur - endGap + delay
        SDend = SDstart + SDdur
...
if __name__ == '__main__':
    ...
    for i in range(1, 2):
        ...
        measurements = "08_04_26_0B_calibration_1"
        filetype : str = ".h5"
        ...
        # ------ Data evaluation ------
        (MWfrequencies, SDmean, err_SDmean) = analysis.dataEv_MWspectrum(path, measurements, filetype)
        ...
        maxima_indices, maxima_values = analysis.find_local_maxima_in_range(np.array(SDmean), window_size_5, threshold)

        print("Indices of local maxima above threshold:", maxima_indices)
        print("Local maxima values above threshold:", maxima_values)
        print("Frequencies of local maxima values above threshold:", [MWfrequencies[i] for i in maxima_indices])
        ...
        if fit_peaks:
            ...
            if n_peaks == 5:
                ...
                l[l < (MWfreqStart - MWfreqCenter)] = MWfreqStart - MWfreqCenter
                r[r > (MWfreqEnd - MWfreqCenter)] = MWfreqEnd - MWfreqCenter - MWfreqStep

                l_index = ((l - (MWfreqStart - MWfreqCenter)) / MWfreqStep).astype(int)
                r_index = ((r - (MWfreqStart - MWfreqCenter)) / MWfreqStep).astype(int)

                print(c, l, r)
                # print(l_index)
                # print(r_index)

                for i in range(len(modMWspec)):
                    print("index = ", i)
                    print(l[i], r[i])

                    (modMWspec[i], res_MWspec[i]) = analysis.dataFit_lorenzian(MWfrequencies[l_index[i]:r_index[i]],
                                                                      SDmean[l_index[i]:r_index[i]],
                                                                      err_SDmean[l_index[i]:r_index[i]])

                freqFitPlot = [np.linspace(MWfrequencies[l_index[i]], MWfrequencies[r_index[i]], 1000) for i in range(len(modMWspec))]

            elif n_peaks == 1:
                l1, c1, r1 = -24, 0, 24  # left1, center1, right1
                l1_index, r1_index = int((l1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep), int(
                    (r1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep)
                print(l1_index, r1_index)

                modMWspec, res_MWspec, c = [0], [0], [c1]
                (modMWspec[0], res_MWspec[0]) = analysis.dataFit_lorenzian(MWfrequencies[l1_index:r1_index],
                                                                      SDmean[l1_index:r1_index],
                                                                      err_SDmean[l1_index:r1_index])

                freqFitPlot = [np.linspace(MWfrequencies[l1_index], MWfrequencies[r1_index], 1000)]

            elif n_peaks == 3:
                l1, c1, r1 = -70, 0, 70  # left1, center1, right1
                l1_index, r1_index = int((l1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep), int(
                    (r1 - (MWfreqStart - MWfreqCenter)) / MWfreqStep)
                print(l1_index, r1_index)
        ...
        plt.close(measurements)
        fig = plt.figure(measurements, figsize=(11, 5.5))
        ax1 = fig.add_subplot(1, 1, 1)

        plt.errorbar(MWfrequencies, SDmean, yerr=err_SDmean, marker = "o", linestyle = "-",color="black", capsize=3)
        if fit_peaks:
            string = measurements + '\n'
            for i in range(len(modMWspec)):
                ax1.plot(freqFitPlot[i], modMWspec[i].eval(freq=freqFitPlot[i], params=res_MWspec[i].params), "r")
                ax1.text(c[i], np.round(res_MWspec[i].best_values['amp1'], 1),
                         np.round(res_MWspec[i].best_values['foff'], 1), fontsize=afs)
                string += 'foff ' + str(i + 1) + ' = ' + str(
                    np.round(res_MWspec[i].best_values['foff'], 1)) + 'kHz' + '       FWHM ' + str(i + 1) + ' = ' + str(
                    np.round(2 * res_MWspec[i].best_values['k1'], 2)) + 'kHz\n'

            # if len(modMWspec) > 1:
            #     Delta = np.round(res_MWspec[2].best_values['foff'], 1) - np.round(res_MWspec[0].best_values['foff'], 1)
            #     Delta_stderr = np.round(res_MWspec[2].params['foff'].stderr, 2) + np.round(res_MWspec[0].params['foff'].stderr,2)
            #     string += '\n' + r'$\Delta$(3-1) = ' + str(Delta) + ' pm ' + str(Delta_stderr) + ' kHz'
        ...
        # ax1.plot(df['MWfreqs'], df['SDmean'], marker="o", linestyle="-", color="black")
        ...
        fig.savefig(path + "\\" + fig.get_label())
