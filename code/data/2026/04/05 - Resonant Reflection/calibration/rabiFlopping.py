...
class AtomAnalysis:
    ...
    def dataEv_postSelection(self, path: str, filename: str, filetype: str = '.h5', kcCounts: int = 2000, no=1):
        ...
        for i in tqdm(atomList, file=sys.stdout):
            ...
            wt_kc = 0.2 * kcCounts  # wt = witness threshold
            wt_lc = -1  # wt = witness threshold
            twot = 2.2 * kcCounts  # twot = two atom threshold
            inAtom = False
            atomIn_index = 0
            atomOut_index = 0
    ...
    def dataEv_MWRabiRot(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts=2500, no = 10)

        a_file = open(file_postSelected, "rb")
        atomDic = pickle.load(a_file)

        # Sequence timings
        coolingDur = 0.5547e-3 - 1.5e-6 + 0*34e-6  # (s) duration of the atom cooling stage
        # OPdur = 200e-6
        OPdur = 340e-6
        ...
        NrPoints = 30  # microwave duration steps per ramp
        # timeIntervalStep = 4e-6  # (s) microwave duration step
        timeIntervalStep = 6e-6  # (s) microwave duration step
...
if __name__ == '__main__':
    ...
    fileList = ["05_04_26_Rabi_0_0_no_lc_5"]
    ...
    if analyze:
    #     for filename in fileList:
        (timeIntervals, SDmean, err_SDmean) = analysis.dataEv_MWRabiRot(path, filename, filetype)
        ...
        # ------ Data fitting ----
        model, fit_results = analysis.dataFit_rabiOsc(df['timeInterval'], df['SDmean'])

        # --- Data Plotting ---
        timeFitPlot = (np.linspace(df['timeInterval'][0], df['timeInterval'][:-1]+10e-6, 1000))
