...
class AtomAnalysis:
    ...
    def dataEv_postSelection(self, path: str, filename: str, filetype: str = '.h5', kcCounts: int = 2000, no = 100):
    ...
    def dataEv_atomPhotonCoincidences(self, path: str, file_list: List[str], filetype: str = '.h5', heralded: bool = True, heraldGate: bool = False):
        ...
        for filename in file_list:
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                fs = dataDic[self.syncFast][1][np.searchsorted(dataDic[self.syncFast][1], fsr)]

                if not heraldGate:  # --- independent on the Herald detection ---
                    # detector lcH
                ...
                else:  # --- using only 20ns time gate depending on Herald detection ---
                    # kcH herald
                    sf = fs + fsdelay[self.kcH]
                    left, right = np.searchsorted(dataDic[self.kcH][1], [sf + excgate[0], sf + excgate[1]])
                    if right - left != 0:
                ...
                if right_cond - left_cond >= 1:
                    ...
                    if ((photonkcH[-1] or photonkcV[-1]) and (photonlcH[-1] or photonlcV[-1])):
                        # if ((photonkcH[-1])):
    ...
    def dataEv_cascadeAtomBasesCalibration(self, path: str, file_list: List[str], filetype: str = '.h5', position: int = 0, heralded: bool = True):
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts= 6200, no = 1)
    ...
    def dataEv_stateDetection(self, path: str, file_list: List[str], filetype: str = 'h5'):
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts=3500)
            ...
            SD1gate = [302.9e-6+800e-6 + 286e-6, 302.0e-6+ 287.2e-6 + 8e-6 +800e-6]
            SD2gate = [613.0e-6+1600e-6+ 286e-6, 612.1e-6 + 287.2e-6+ 8e-6 +1600e-6]
            # SD2gate = SD1gate
            # SD2gate = [2221.45e-6, 2221.45e-6 + 8e-6]
            ...
            # Loop over all the fast sequence triggers
            for fs in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                # SD 1
                sf = fs + fsdelay[self.kcH]
                left_h, right_h = np.searchsorted(dataDic[self.kcH][1], [sf + SD1gate[0], sf + SD1gate[1]])
                sf = fs + fsdelay[self.kcV]
                ...
                if (right_h - left_h > 0) and (right_v - left_v > 0):
                    photonSD_kcH.append(dataDic[self.kcH][1][left_h:right_h] - fsdelay[self.kcH])
                    photonSD_kcV.append(dataDic[self.kcV][1][left_v:right_v] - fsdelay[self.kcV])
        ...
        # time resolved SD counts on SD2
        trSD = []
        for i in range(len(photonSD_kcH)):

            for timekcH in photonSD_kcH[i]:
                for timekcV in photonSD_kcV[i]:
                    trSD.append(timekcH - timekcV)
        ...
        fig = plt.figure("State Detection - " + filename, figsize=[6, 4], constrained_layout=False)
        ax1 = fig.add_subplot(2, 1, 1)
        ax2 = fig.add_subplot(2, 1, 2, sharex=ax1)
        fig.subplots_adjust(hspace=0.05)  # adjust space between Axes
        historange = [0, 35]
        x = np.linspace(0, historange[-1], 35)
        histSD1, bin_edgesSD1 = np.histogram(photonSD1[0], range=historange, bins=len(x))
        ax1.bar(x, histSD1 / np.sum(histSD1), color='b', align='center', width=-0.6, alpha = 0.5)
        ax2.bar(x, histSD1 / np.sum(histSD1), color='b', align='center', width=-0.6, alpha = 0.5)
        histSD2, bin_edgesSD2 = np.histogram(photonSD2[0], range=historange, bins=len(x))
        ax2.bar(x, histSD2 / np.sum(histSD2), color=mcolors.CSS4_COLORS['deepskyblue'], width=0.6, alpha = 0.5)
        ax1.vlines(0.5, ymin=0, ymax=1, ls='--', color='k')
        ax2.vlines(0.5, ymin=0, ymax=1, ls='--', color='k')
        ...
        ax2.set_xlabel('State detection counts')
        ax1.set_ylabel('Probability')
        ax2.set_xlim([-1, 15])

        ax1.yaxis.set_label_coords(-0.2, -0.15)
        ...
        # time resolved counts
        bin_width = 3.33 * 1e-9
        historange = np.array([-0.1,0.1])*1e-6
        histoBinNum = int(((historange[1] - historange[0])) / bin_width)
        x = np.linspace(historange[0], historange[1], histoBinNum)
        histtrSD, bin_edges = np.histogram(trSD, range=historange, bins=len(x))
        fig1 = plt.figure("Time resolved State Detection", figsize=[6, 4], constrained_layout=True)
        ax1 = fig1.add_subplot(1, 1, 1)
        ax1.bar(x*1e9, histtrSD, color='pink', width = bin_width*1e9)
        ax1.set_xlim(historange[0]*1e9, historange[1]*1e9)
...
if __name__ == '__main__':
    """
    This script analyzes a the State Detection.
    """
    ...
    measurements = ["05_04_26_SD_110MHz_1"]
    filetype : str = ".h5"
    ...
    # ------ Data evaluation ------
    histSD1, bin_edgesSD1=analysis.dataEv_stateDetection(path, measurements, filetype)

    # measurements = ["19_02_26_SD_wfeedback_wEOM_600V_121MHz_1"]
    # filetype : str = ".h5"
    # # fileSave : str = "coincidences_" + "19_11_24_tomography_3" + "_kcH"
    #
    # # ------ Begin Analysis ------
    # analysis = AtomAnalysis()
    # # ------ Data evaluation ------
    # histSD2, bin_edgesSD2=analysis.dataEv_stateDetection(path, measurements, filetype)
    ...
    # historange = [0, 35]
    # x = np.linspace(0, historange[-1], 35)
    # ax1.bar(x, histSD1 / np.sum(histSD1), color='b', align='center', width=-0.6)
    # ax2.bar(x, histSD1 / np.sum(histSD1), color='b', align='center', width=-0.6)
    # ax2.bar(x, histSD2 / np.sum(histSD2), color=mcolors.CSS4_COLORS['darkorange'], width=0.6, alpha=0.7)
    # ax1.vlines(0.5, ymin=0, ymax=1, ls='--', color='k')
    # ax2.vlines(0.5, ymin=0, ymax=1, ls='--', color='k')
    # # ax1.set_title("State Detection Histogram")
    # ax1.set_ylim(.98, 1.)  # outliers only
    # ax2.set_ylim(0, .25)  # most of the data
    # ax1.set_yticks([0.99, 1])
