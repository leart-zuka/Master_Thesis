...
class AtomAnalysis:
        ...
        for filename in file_list:
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                fs = dataDic[self.syncFast][1][
                    np.searchsorted(dataDic[self.syncFast][1], fsr)
                ]
                ...
                if right_cond - left_cond >= 1:
                    ...
                    if (photonkcH[-1] or photonkcV[-1]) and (
                        photonlcH[-1] or photonlcV[-1]
                    ):
                        # if ((photonkcH[-1])):
    ...
    def dataEv_stateDetection(
        ...
        for filename in file_list:
            ...
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts=2000)
            ...
            SD1gate = [
                3.15e-6 + 400e-6 + 340e-6 + 37.9e-6,
                3.15e-6 + 400e-6 + 340e-6 + 37.9e-6 + 8e-6,
            ]
            SD2gate = [
                3.15e-6 + 400e-6 + 340e-6 + 47.5e-6,
                3.15e-6 + 400e-6 + 340e-6 + 47.5e-6 + 8e-6,
            ]
            # SD2gate = SD1gate
            ...
            for fs in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                ...
                if (right_h - left_h > 0) and (right_v - left_v > 0):
                    photonSD_kcH.append(
                        dataDic[self.kcH][1][left_h:right_h] - fsdelay[self.kcH]
                    )
                    photonSD_kcV.append(
                        dataDic[self.kcV][1][left_v:right_v] - fsdelay[self.kcV]
                    )
        ...
        print("correct detection of F2 P(F2|n=0) = %.3f pm %.4f" % (fF1, fF1_err))
        print("correct detection of F2 P(F2|n>0) = %.3f pm %.4f " % (fF2, fF2_err))
        print(
            f"Scattering from gate photon is: {(fF1 - fF2) * 100:.2f}+/-{np.sqrt((fF1_err / (fF1)) ** 2 + (fF2_err / fF2) ** 2) * (fF1 - fF2) * 100:.2f}%"
        )
        ...
        fig = plt.figure(
            "State Detection - " + filename, figsize=[6, 4], constrained_layout=False
        )
        ...
        ax2.set_ylim(0, 0.25)  # most of the data
        ...
        fig1 = plt.figure(
            "Time resolved State Detection", figsize=[6, 4], constrained_layout=True
        )
...
if __name__ == "__main__":
    ...
    measurements = ["06_04_26_Scattering_372nW_before_cav_1mus_pulse_sending_L_1"]
    filetype: str = ".h5"
