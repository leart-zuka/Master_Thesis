...
class fittingFunctions:
    ...
    def rabiRotation(self, time, freq, decRate, A, off):
        ...
        p1 = A * ((np.sin(2 * np.pi * freq * 1e3 * time)**2-0.5)*np.exp(-decRate * 1e3 * time) + 0.5) + off
    ...
    def ramseyRotation(self, time, freq, decRate, A, off):
        ...
        p1 = A * ((np.cos(2 * np.pi * freq * 1e3 * time)**2-0.5)*np.exp(-decRate * 1e3 * time) + 0.5) + off

        return p1

    def ramseyOscillation(self, time, freq, A, off):
        """
        INPUTS
        time: duration of the transfer pulse
        rabiFreq: Rabi frequency
        ...
        p1 = A * (np.cos(2 * np.pi * freq * 1e3 * time)**2-0.5) + off
...
class AtomAnalysis:
    ...
    def dataEv_MWRamseySequence(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts=2200, no = 30)

        a_file = open(file_postSelected, "rb")
        atomDic = pickle.load(a_file)

        # Sequence timings
        coolingDur = 1.0561e-3 + 29.6*1e-6*2  # (s) duration of the atom cooling stage
        OPdur = 200e-6
        SDdur = 7.5e-6  # duration of the state detection stage
        endGap = 2e-6

        timeIntervalStart = 1986 * 1e-6  # (s) initial microwave duration
        NrPoints = 27  # microwave duration steps per ramp
    ...
    def dataFit_ramseyOsc(self, x, y):
        ...
        model = lmfit.Model(fit.ramseyOscillation)  # model used for fitting
        params = model.make_params()
        ...
        # Parameters for the rabi Oscillation fit (label,start,lowerLimit,upperLimit, fittingBoolean)
        pset("freq", 24.38, 1, 200, True)
        pset('A', 0.8, 0.001, 2, True)
        pset('off', 0.5, 0.0, 0.6, True)
...
if __name__ == '__main__':
    ...
    fileList = ["21_11_25_Ramsey_SpinEcho3_2ms_1"]
    ...
    if analyze:
    #     for filename in fileList:
        (timeIntervals, SDmean, err_SDmean) = analysis.dataEv_MWRamseySequence(path, filename, filetype)
        ...
        # ------ Data fitting ----
        model, fit_results = analysis.dataFit_ramseyOsc(df['timeInterval'], df['SDmean'])

        # --- Data Plotting ---
        timeFitPlot = (np.linspace(0, df['timeInterval'][:-1]+200e-6, 1000))
        plt.rcParams.update({'font.size': 15})

        fig1 = plt.figure(str(filename), figsize=(15,8), constrained_layout=True)
        ...
        print()
        fig1.suptitle(filename + report[index + len("[[Variables]]"):] + "\n" + "Pi pulse: %.1f us"%(pipulse*1e3))
        ax1 = fig1.add_subplot(1, 1, 1)
        ax1.plot(timeFitPlot * 1e6, model.eval(time=timeFitPlot, params=fit_results.params), color=analysis.colour["blueLight"],zorder=-1)
        ax1.errorbar(df['timeInterval'] * 1e6, df['SDmean'], yerr=df['err_SDmean'], marker="o", linestyle="", color=analysis.colour["blueDark"], capsize=3)
