...
if __name__ == "__main__":
    ...
    if analyze:
        #     for filename in fileList:
        (timeIntervals, SDmean, err_SDmean) = analysis.dataEv_MWRabiRot(
        ...
        pipulse_err = (
            1 / (4 * fit_results.best_values["freq"] ** 2)
        ) * fit_results.params["freq"].stderr
        index = report.find("[[Variables]]")
        print(index)
        print(report[index + len("[[Variables]]") :])
        print(pipulse * 1e3)
        print(pipulse_err * 1e3)

        # --- Data Plotting ---
        timeFitPlot = np.linspace(
            df["timeInterval"].iloc[0], df["timeInterval"].iloc[-1] + 10e-6, 1000
        )
        ...
        fig1, ax1 = plt.subplots(figsize=(5.5, 3.2), constrained_layout=True)
        ...
        ax1.set_xlabel(r"Microwave pulse duration ($\mu$s)")
        ax1.set_ylabel(r"Population in F=2")
        ax1.set_ylim([0, 1])
        ax1.set_xlim([0, 185])
