...
class AtomAnalysis:
    ...
    def dataEv_MWRamseySequence(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts=1500, no = 30)

        a_file = open(file_postSelected, "rb")
        atomDic = pickle.load(a_file)

        # Sequence timings
        coolingDur = 1.0550e-3 + 28.6*1e-6  - 1*1e-6# (s) duration of the atom cooling stage
        OPdur = 200e-6
...
if __name__ == '__main__':
    ...
    fileList = ["13_02_26_MWRamsey_0_m1_1086us_feedforward_50Hz_3mV_0ms_1"]
