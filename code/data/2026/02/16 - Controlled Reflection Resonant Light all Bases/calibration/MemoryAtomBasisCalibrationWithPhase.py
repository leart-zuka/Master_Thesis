...
class AtomAnalysis:
    ...
    def dataEv_atomPhotonCoincidences(self, path: str, file_list: List[str], filetype: str = '.h5', position: int = 0, heralded: bool = True):
        ...
        for filename in file_list:
            ...
            # excgate = [opgate[1] + gap + 7.38 * 1e-6, opgate[1] + gap + 7.38 * 1e-6 + 400e-9]
            # excgate = [10.165389e-3, 10.165389e-3 + 400e-9]
            excgate = [5.165389e-3, 5.165389e-3 + 400e-9]

            # SD and coincidence gates after "wait"  time
            # SDgate = [40.26886e-3 + wait, 40.26886e-3 + wait + SD]
            # coincgate = [40.265e-3 + wait, 40.27e-3 + wait]
            # SDgate = [10.26886e-3 + wait, 10.26886e-3 + wait + SD]
            # coincgate = [10.265e-3 + wait, 10.27e-3 + wait]
            SDgate = [5.26886e-3 + wait, 5.26886e-3 + wait + SD]
            coincgate = [5.265e-3 + wait, 5.27e-3 + wait]
            hgate = [-20e-9, +20e-9]
    ...
    def dataEv_cascadeAtomBasesCalibration(self, path: str, file_list: List[str], filetype: str = '.h5', position: int = 0, heralded: bool = True):
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts= 5500, no = 1)
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                ...
                for i in range(8):
                    ...
                    if right_cond - left_cond >= 1:
                        ...
                        # if ((photonlcH[-1] or photonlcV[-1]) and (photonkcV[-1])):
                            ...
                            if (photonlcH[-1] and (photonSD[0][-1] == 0)):
                                sf = fs + fsdelay[self.lcH]
                                left, right = np.searchsorted(dataDic[self.lcH][1], [sf + excgate[0], sf + excgate[1]])
                                (tlcHF1.extend(dataDic[self.lcH][1][left:right] - sf) if left - right != 0 else None)
                            elif (photonlcH[-1] and (photonSD[0][-1] > 0)):
                                sf = fs + fsdelay[self.lcH]
                                left, right = np.searchsorted(dataDic[self.lcH][1], [sf + excgate[0], sf + excgate[1]])
                                (tlcHF2.extend(dataDic[self.lcH][1][left:right] - sf) if left - right != 0 else None)
                            elif (photonlcV[-1] and (photonSD[0][-1] == 0)):
                                sf = fs + fsdelay[self.lcV]
                                left, right = np.searchsorted(dataDic[self.lcV][1], [sf + excgate[0], sf + excgate[1]])
                                (tlcVF1.extend(dataDic[self.lcV][1][left:right] - sf) if left - right != 0 else None)
                            elif (photonlcV[-1] and (photonSD[0][-1] > 0)):
                                sf = fs + fsdelay[self.lcV]
                                left, right = np.searchsorted(dataDic[self.lcV][1], [sf + excgate[0], sf + excgate[1]])
                                (tlcVF2.extend(dataDic[self.lcV][1][left:right] - sf) if left - right != 0 else None)
        ...
        # histogram_lc = np.histogram(tlcH + tlcV, range=historange, bins=histoBinNum)[0]
        histogram_lc_good = np.histogram(tlcHF1 + tlcVF2, range=historange, bins=histoBinNum)[0]
        histogram_lc_bad = np.histogram(tlcHF2 + tlcVF1, range=historange, bins=histoBinNum)[0]
        histogram_kc = np.histogram(tkcH + tkcV, range=historange, bins=histoBinNum)[0]
        ...
        ax1.bar(histotime, histogram_lc_good / fstot, width=bin_width * 1e9, color='b', alpha = 0.6, label="good")
        ax1.bar(histotime, histogram_lc_bad / fstot, width=bin_width * 1e9, color='r', alpha = 0.6, label="bad")
        ax = phfig.add_subplot(2, 1, 2)
        ax.bar(histotime, histogram_kc / fstot, width=bin_width * 1e9, color="orange", label="kcPhoton")
    ...
    def dataEv_memoryAtomBasesCalibration(self, path: str, file_list: List[str], filetype: str = '.h5'):
        ...
        photonSD = [[], []]  # n of photons in SD gate for each trial
        steps = 11
        kcHF1, kcHF2, kcVF1, kcVF2 = np.array([0 for i in range(steps)]), np.array([0 for i in range(steps)]), np.array([0 for i in range(steps)]), np.array([0 for i in range(steps)])
        fstot = 0
        sdTrigtot = 0
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts= 60, no = 300)
            ...
            # sequence parameters
            analysis_flag = 'external_trigger_awg'
            ...
            if analysis_flag == 'external_trigger_awg':
                ...
                SDgate = [444.47e-6, 444.47e-6 + SDDur]
                coincgate = [404e-6, 405e-6]
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                ...
                for i in range(steps):
                    ...
                    left_h, right_h = np.searchsorted(dataDic[self.kcH][1], [sf + SDgate[0], sf + SDgate[1]])
                    left_v, right_v = np.searchsorted(dataDic[self.kcV][1], [sf + SDgate[0], sf + SDgate[1]])
                    left_cond, right_cond = np.searchsorted(dataDic[self.sdTrig][1],
                                                            [sf + coincgate[0], sf + coincgate[1]])
                    ...
                    if right_cond - left_cond >= 1:
                        ...
                        # heralded coincidences
                        kcHF1[i] += (photonkcH[-1] & (photonSD[0][-1] == 0))
                        kcHF2[i] += (photonkcH[-1] & (photonSD[0][-1] >= SDthreshold))
                        # kcVF1[i] += (photonkcV[-1] & (photonSD[0][-1] == 0))
                        # kcVF2[i] += (photonkcV[-1] & (photonSD[0][-1] >= SDthreshold))
                        kcVF1[i] += 0
                        kcVF2[i] += 0
        ...
        effkcText = "Photon detection effciency kC = %.2f(%d) %%" % (photonkcEff * 1e2, photonkcEfferr * 1e4)
        ...
        histotime = (np.linspace(historange[0], historange[1], histoBinNum) - historange[0]) * 1e9  # ns
        ax.bar(histotime, histogram_kc / fstot, width=bin_width * 1e9, color="red", label="herald")
        ...
        spttl = filename + " - State Detection"
        sdfig = plt.figure(spttl, figsize=[10, 7])
        sdfig.suptitle(filename + "\nbin width= %.1f ns" % (bin_width * 1e9))
        ax = sdfig.add_subplot(1, 1, 1)
        histotime = (np.linspace(historange[0], historange[1], histoBinNum) - historange[0]) * 1e9  # ns
    ...
    def dataEv_writeAtom(self, path: str, file_list: List[str], filetype: str = 'h5', position: int = 0):
        ...
        for filename in file_list:
            ...
            file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(path, filename, filetype, kcCounts= 2200, no = 50)
    ...
    def dataFit_atomPrecession(self, x, y):
        ...
        # Parameters for the rabi Oscillation fit (label,start,lowerLimit,upperLimit, fittingBoolean)
        pset("freq", 1, 0, 200, False)
        pset("x0", 1.5, -5, 5, True)
        pset('a', 0.45, 0.001, 1, True)
        pset('off', 0.00, -1, 1, True)
...
if __name__ == '__main__':
    """
    This script analyzes a 2-qubit Tomography.
    It computes the measured density matrix and the Entanglement Fidelity of this 2-qubit state.
    """
    ...
    # ------ Setting flags------
    loadFromDir : bool = False

    # ------ Definition of data sources and destinations------
    path : str = os.path.abspath(os.path.dirname(__file__))

    measurements = ["15_01_26_Memory_writeV_basisCalibration_60us_1"]

    filetype : str = ".h5"
    fileSave : str = "" + measurements[0]
    ...
    # times = np.linspace(0, 7*0.71, 8)
    times = np.linspace(0, 0.1*10, 11)
    counts = {'F1' : np.array([]), 'F2' : np.array([])}
    ...
    if loadFromDir:
        ...
        counts = analysis.dataEv_memoryAtomBasesCalibration(path, measurements, filetype)
        print(counts)
    ...
    pF2 = counts['F2']/(counts['F2']+counts['F1'])
    pF2std = np.sqrt(counts['F2'])/(counts['F2']+counts['F1'])
    pF1 = counts['F1'] / (counts['F2'] + counts['F1'])
    pF1std = np.sqrt(counts['F1']) / (counts['F2'] + counts['F1'])


    # ------ Data fitting ----
    model_pF2, fit_results_pF2 = analysis.dataFit_atomPrecession(times, pF2)
    report_pF2 = (fit_results_pF2.fit_report(show_correl=False))
    timeFitPlot = np.linspace(times[0], times[-1], 1000)

    # --- Data Plotting ---
    plt.rcParams.update({'font.size': 15})

    f = plt.figure(fileSave + "_pF2", figsize=[10, 7])
    index = report_pF2.find("[[Variables]]")
    f.suptitle(fileSave + report_pF2[index + len("[[Variables]]"):] + "\n")
    ax = f.add_subplot(111)
    ax.errorbar(times, pF2, pF2std, ls="", marker='o', markersize=5, capsize=5, color='k')
    ax.plot(timeFitPlot, model_pF2.eval(x=timeFitPlot, params=fit_results_pF2.params), color='r', zorder=-1)
    ax.set_xlabel(r'Phase (rad)')
    ax.set_ylabel('p(F2)')
    ax.set_ylim(0, 1)
    plt.tight_layout()

    f.savefig(path + "\\" + f.get_label() + ".pdf")
    f.savefig(path + "\\" + f.get_label())
