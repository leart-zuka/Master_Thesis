...
class AtomAnalysis:
    ...
    def dataEv_writeAtom(
        ...
        for filename in file_list:
            ...
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(
                    path, filename, filetype, kcCounts=1700, no=30
                )
            ...
            if analysis_flag == "local":
                ...
                noisegate = [
                    trigger_delay + OPdur + trapOffdelay + 2e-6,
                    trigger_delay + OPdur + trapOffdelay + noiseDur + 2e-6,
                ]
                writegate = [0.06194e-3, 0.06205e-3]
                writegate = [0.06190e-3, 0.06205e-3]
                noisegate = [0.0536e-3, 0.0536e-3 + noiseDur]
                gates = writegate + SDgate + coincgate + noisegate

            elif analysis_flag == "qgate":
                seqDur = 1.3e-3

                trigger_delay = 3.15e-6
                OPdur = 50e-6
                com_time = 2.56e-6 + 2e-6 + 8.7e-6
                pulse_duration = 150e-9
                MWpulse_delay = 2.50e-6 + 1.4e-6 + 1.2e-6
                MWpulse1 = 29.6e-6  # 100kHz Lamor frequency
                MWpulse2 = 53.4e-6 / 2  # 100kHz Lamor freqency
                # MWpulse1 = 13.8e-6 #300kHz Lamor frequency
                # MWpulse2 = 25e-6/2 #300kHz lamor frequency
                SDdur = 8e-6
                coinc_delay = 6.9e-6 - 8.7e-6 + 3.4e-6
                trapOffdelay = 1.5e-6
                noiseDur = 5e-6
                photonDur = 0.3e-6
                ...
                com_time_photon = (
                    6.59e-6 + 2.245e-6
                )  # additional delay of 2.23us due to considering trigger from LMU
                ...
                photongate = [
                    opgate[1] + com_time_photon,
                    opgate[1] + com_time_photon + photonDur,
                ]
                ...
                noisegate = [
                    trigger_delay + OPdur + trapOffdelay,
                    trigger_delay + OPdur + trapOffdelay + noiseDur,
                ]
                ...
                writegate = [
                    opgate[1] - ext_trigger_delay,
                    opgate[1] + pulseDur - ext_trigger_delay,
                ]
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                fs = dataDic[trig][1][
                    np.searchsorted(dataDic[trig][1], fsr) + position
                ]  # + position*1e-6
                # kcH herald
                sf = fs + fsdelay[self.kcH]
                left, right = np.searchsorted(
                ...
                if analysis_flag == "local":
                    ...
                    sf = fs + fsdelay[self.lcH]
                    left, right = np.searchsorted(
                        dataDic[self.lcH][1], [sf + writegate[0], sf + writegate[1]]
                    )
                    (
                        tlcH.extend(dataDic[self.lcH][1][left:right] - sf)
                        if left - right != 0
                    ...
                    sf = fs + fsdelay[self.lcV]
                    left, right = np.searchsorted(
                        dataDic[self.lcV][1], [sf + writegate[0], sf + writegate[1]]
                    )
                    (
                        tlcV.extend(dataDic[self.lcV][1][left:right] - sf)
                        if left - right != 0
                ...
                # Noise
                left_h, right_h = np.searchsorted(
                    dataDic[self.kcH][1], [sf + noisegate[0], sf + noisegate[1]]
                )
                # left_v, right_v = np.searchsorted(dataDic[self.kcV][1], [sf + noisegate[0], sf + noisegate[1]])
                photonNoise[0].append((right_h - left_h))
                (
                    photonNoise[1].extend(dataDic[self.kcH][1][left_h:right_h] - sf)
                    if left_h - right_h != 0
                    else None
                )
                # (photonNoise[1].extend(dataDic[self.kcV][1][left_v:right_v] - sf) if left_v - right_v != 0 else None)
        ...
        if analysis_flag == "local":
            photonlcEff, photonlcEfferr = (
                (sum(photonlcV) + sum(photonlcH)) / fstot,
                np.sqrt(sum(photonlcV) + sum(photonlcH)) / fstot,
            )
            print("============================================")
            print("             Reflection analysis")
            print("============================================")
            print(
                "Number of reflected photons = %.0f" % (sum(photonlcH) + sum(photonlcV))
            )
            print(
                "Efficiency reflection total = %.7f(%.5f)"
                % (photonlcEff, photonlcEfferr)
            )
        print("============================================")
        ...
        def emg(x, A, mu, sigma, lambd):
            ...
            """
            conv = (lambd / 2) * np.exp(
                (lambd / 2) * (2 * mu + lambd * sigma**2 - 2 * x)
            )
        ...
        popt, pcov = curve_fit(
            gaussian, histotime, histogram_kc / fstot, p0=p0, bounds=bounds
        )
        ...
        ax.bar(
            ...
            width=bin_width * 1e9,
            color="red",
            label="Herald photon data",
        )
        ax.plot(
            model_time,
            gaussian(model_time, *popt),
            label="Gaussian fit",
            color="blue",
            linewidth=2,
            linestyle="--",
        ...
        if analysis_flag == "local":
            histogram_lc = np.histogram(
                tlcH + tlcV, range=historange, bins=histoBinNum
            )[0]
