...
def plot_entire_poincare_sphere_with_all_pols(
    df_h,
    df_v,
    df_a,
    df_d,
    ...
    ax.scatter(S1_v, S2_v, S3_v, color="lightgreen", s=80, label="Start V")
    ax.scatter(S1_a, S2_a, S3_a, color="peachpuff", s=80, label="Start A")
    ax.scatter(S1_d, S2_d, S3_d, color="peachpuff", s=80, label="Start D")
    ax.scatter(S1_r, S2_r, S3_r, color="lightcoral", s=80, label="Start R")
    ax.scatter(S1_l, S2_l, S3_l, color="lightcoral", s=80, label="Start L")
    ...
    # plt.tight_layout()
    plt.title("Starting Polarizations")
    plt.legend()
    plt.savefig("starting_polarizations.svg")
    plt.show()


plot_entire_poincare_sphere_with_all_pols(df_h, df_v, df_a, df_d, df_r, df_l)
