...
comparisson = "transmission through 2nd dichroic with compensation for dichroics"
df_h_compare = pd.read_csv(
    "./H_transmission_2nd_dichroic_compensation.csv",
    sep=";",
    skiprows=8,
)
df_v_compare = pd.read_csv(
    "./V_transmission_2nd_dichroic_compensation.csv",
    sep=";",
    skiprows=8,
)
...
comparisson = "Reflection from cavity"
df_h_compare = pd.read_csv(
    "./H_reflection.csv",
    sep=";",
    skiprows=8,
...
plot_average_poincare_point(
    df_h, df_v, df_h_compare, df_v_compare, comparisson, show_plot=True
)


def calculate_percentage_of_light_in_h_when_sending_v(
    azimuth: float, ellipticity: float
):
    s_1 = np.cos(2 * azimuth) * np.cos(2 * ellipticity)
    print(s_1)
    return (1 - s_1) / 2


print(df_v_compare[" Normalized s 1 "].mean())

print(
    f"{
        calculate_percentage_of_light_in_h_when_sending_v(
            df_v_compare[' Azimuth[°] '].mean(), df_v_compare[' Ellipticity[°] '].mean()
        ):.2f}%"
)
