...
df = pd.read_csv("nms_errorbar_data_3.csv")
...
ax.errorbar(
    ...
    marker="o",
    color="blue",
    label="Measurement data AOM @ 100",
)

ax.plot(
    x,
    R_coupled_star(x, *popt),
    label=f"Model fit: {popt[0]:.2f}+/-{pcov[0]:.2f}, w AOM @ 100",
    color="red",
    linewidth=3,
    linestyle="-.",
)
...
# --- load the CSV you saved earlier ---
df = pd.read_csv("nms_errorbar_data_2.csv")
...
ax.errorbar(
    ...
    color="orange",
    marker="o",
    label="Measurement data AOM @ 95",
)

ax.plot(
    x,
    R_coupled_star(x, *popt),
    label=f"Model fit: {popt[0]:.2f}+/-{pcov[0]:.2f}, w AOM @ 95",
    color="green",
    linewidth=3,
    linestyle="-.",
)
