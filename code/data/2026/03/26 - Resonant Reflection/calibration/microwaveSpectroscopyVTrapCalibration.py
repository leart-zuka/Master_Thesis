...
class DataLoading:
    ...
    def data_loading(self, path: str, fileName: str, atom: int):
        ...
        """
        dataDic = {'ch0': [0, []], 'ch1': [1, []], 'ch2': [2, []], 'ch3': [3, []], 'ch4': [4, []], 'ch5': [5, []],'ch6': [6, []], 'ch7': [7, []]}

        filedata = h5py.File(path + '\\' + fileName + ".h5", mode="r")

        for ch in dataDic.keys():
            # We put the data into the dictionary
            dataDic[ch][1] = (filedata["atom_" + str(atom) + "_" + str(dataDic[ch][0])] * filedata.attrs["qu_tau_timebase"])
    ...
    def data_goodAtoms(self, path: str, fileName: str, atomDic: dict):
        ...
        """
        dataVD = {'ch0': [0, []], 'ch1': [1, []], 'ch2': [2, []], 'ch3': [3, []], 'ch4': [4, []], 'ch5': [5, []], 'ch6': [6, []], 'ch7': [7, []]}
        ...
        for atom in tqdm(atomDic.keys(), file=sys.stdout):
            ...
            right_sf = np.searchsorted(dataD['ch5'][1], dataD['ch0'][1][0] + atomDic[atom][1])  # index of the first syncFastScan that we don't consider
            left_sf = np.searchsorted(dataD['ch5'][1], dataD['ch0'][1][0] + atomDic[atom][0])  # index of the first syncFastScan that we consider
...
class AtomAnalysis:
    ...
    def dataEv_postSelection(self, path: str, filename: str, filetype: str = '.h5', kcCounts: int = 2000, no=1):
        ...
        for i in tqdm(atomList, file=sys.stdout):
            ...
            twot = 1.8 * kcCounts  # twot = two atom threshold
            inAtom = False
            atomIn_index = 0
            atomOut_index = 0
        ...
        if self.psSave == True:
            ...
            print('Average single atom trapping time: (%.2f +/- %.2f)s' % (
                ...
                np.std(list_trappingDuration) / np.sqrt(np.size(list_trappingDuration))))
    ...
    def dataEv_atomPhotonCoincidences(self, path: str, file_list : List[str], filetype : str = '.h5', position: int = 0, heralded: bool = True):
        ...
        for filename in file_list:
            ...
            excgate =  [opgate[1] + gap + 7.38*1e-6, opgate[1] + gap + 7.38*1e-6 + 600e-9]
            MWgate = [opgate[1] + 3*gap + excdur, opgate[1] + 3*gap + excdur + MW +50e-6]
            ...
            trigger = self.syncFast
            maxTrigDiff = seqDur + 20e-3# should be bigger than triggers time difference
            binsize = 0.5 * 1e-6
            ...
            for fsr in tqdm(dataDic[trig][1][:-5], file=sys.stdout):
                fs = dataDic[self.syncFast][1][np.searchsorted(dataDic[self.syncFast][1], fsr) + position] # + position*1e-6
        ...
        ax1.bar(histotime, histogram_lc/fstot, width = bin_width*1e9, color=mcolors.CSS4_COLORS['cornflowerblue'], label="lcPhoton")
        ...
        errg2_kc = np.clip(coincidences_kc,1,1e15)/np.average(np.delete(coincidences_kc,10))*np.sqrt(1/np.clip(coincidences_kc,1,1e15)+1/np.sum(np.delete(coincidences_kc,10)))
        errg2_lc = np.clip(coincidences_lc,1,1e15)/np.average(np.delete(coincidences_lc,10))*np.sqrt(1/np.clip(coincidences_lc,1,1e15)+1/np.sum(np.delete(coincidences_lc,10)))
        ...
        if self.dataSave:
            ...
            # We add the relevant parameters to a data frame
            efficiencyDF['# lcH'], efficiencyDF['# lcV'], efficiencyDF['# lc'] = [sum(photonlcH)], sum(photonlcV), sum(photonlcH) + sum(photonlcV)
            efficiencyDF['# kcH'], efficiencyDF['# kcV'], efficiencyDF['# kc'] = sum(photonkcH), sum(photonkcV), sum(photonkcH) + sum(photonkcV)
    ...
    def dataEv_MWRabiRot(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        for sfs in tqdm(dataDic[syncRamp][1][:-1], file=sys.stdout):
            ...
            for step in range(0, NrPoints):
                ...
                left_h, right_h = np.searchsorted(dataDic[self.kcH][1], [sf + fsdelay[self.kcH] + SDstart[step], sf + fsdelay[self.kcH] + SDend[step]])
                left_v, right_v = np.searchsorted(dataDic[self.kcV][1], [sf + fsdelay[self.kcV] + SDstart[step],  sf + fsdelay[self.kcV] + SDend[step]])
    ...
    def dataEv_MWspectrum(self, path: str, filename : str, filetype : str = 'h5'):
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts = 2600, no = 100)
        ...
        for sfs in tqdm(dataDic[syncRamp][1][:-2], file=sys.stdout):
            ...
            for step in range(0, MWfreqStepsPerRamp, 1):
                ...
                right_v = np.searchsorted(dataDic[self.kcV][1], dataDic[trigger][1][indexSF] + SDend + fsdelay[self.kcV])
    ...
    def dataEv_MWspectrumTrapScan(self, path: str, filename : str, filetype : str = 'h5', trapPower : List[float] = [130]):
        """
        # INPUTS
        # path: directory where to find the dataDic
        # filename: name of the file to be analyzed
        ...
        file_postSelected = path + '\\goodAtomSelectorFiles\\' + filename + "_goodAtoms.pkl"
        if not os.path.exists(file_postSelected):
            self.dataEv_postSelection(path, filename, filetype, kcCounts = 1300, no = 200)
        ...
        MWfreqCenter = 0  # in kHz
        # MWfreqSpan = 300  # in kHz
        MWfreqSpan = 250 # in kHz
        MWfreqStart = MWfreqCenter - MWfreqSpan / 2  # (KHz) initial microwave duration
        MWfreqEnd = MWfreqCenter + MWfreqSpan / 2  # (KHz) initial microwave duration
        # MWfreqStepsPerRamp = 600  # number of microwave  steps per ramp
        MWfreqStepsPerRamp = 60  # number of microwave  steps per ramp
        MWfreqStep = 1  # (kHz) microwave step
        ...
        # ------ Simple calculations of parameters ------
        # Calculation of the microwave frequencies
        MWfrequencies_interval1 = np.linspace(MWfreqStart, MWfreqStart+int(MWfreqStepsPerRamp/2)-1, int(MWfreqStepsPerRamp/2))
        MWfrequencies_interval2 = np.linspace(MWfreqEnd-int(MWfreqStepsPerRamp/2), MWfreqEnd-1, int(MWfreqStepsPerRamp/2))

        MWfrequencies = np.concatenate((MWfrequencies_interval1, MWfrequencies_interval2))
        # MWfrequencies = MWfrequencies_interval1
        print(MWfrequencies)
        print(len(MWfrequencies))

        # Calculation of the state detection windows
        SDstart = 1283.9e-6
        SDend = 1284.9e-6 + SDdur
        seqDur = SDend
        ...
        specDuration = seqDur * MWfreqStepsPerRamp  # s
        print("Duration 1 Spectrum = %.3f s" % (specDuration))
        print("Duration 1 Trap Power Scan = %.3f s" % (specDuration * len(trapPws)))
        # atomDic = self.load.atomDicCleaner(pickle.load(a_file), specDuration)
        print(atomDic)
        ...
        SDmean = [0 for i in range(len(trapPower))]
        err_SDmean = [0 for i in range(len(trapPower))]

        # Loop over different trap powers
        for i in range(len(trapPower)):

            # Initilization of state detection arrays: each element corresponds to a trial, 0=dark, 1=bright
            SD_binary = dict((i, []) for i in range(MWfreqStepsPerRamp))

            for pfs in tqdm(dataDic[self.syncFast2][1][:-3], file=sys.stdout):

                indexSFS0 = np.searchsorted(dataDic[self.sdTrig][1], pfs) + i

                # Loop over different scan triggers

                indexSF0 = np.searchsorted(dataDic[self.syncFast][1], dataDic[self.sdTrig][1][indexSFS0])
                # print(indexSF0)
                for step in range(0, MWfreqStepsPerRamp, 1):
                    # iterate through all steps in a spectrum

                    indexSF = indexSF0 + step * 1
                    left_h = np.searchsorted(dataDic[self.kcH][1], dataDic[self.syncFast][1][indexSF] + SDstart)
                    right_h = np.searchsorted(dataDic[self.kcH][1], dataDic[self.syncFast][1][indexSF] + SDend)
                    left_v = np.searchsorted(dataDic[self.kcV][1], dataDic[self.syncFast][1][indexSF] + SDstart)
                    right_v = np.searchsorted(dataDic[self.kcV][1], dataDic[self.syncFast][1][indexSF] + SDend)
            ...
            SDmean[i] = np.zeros(MWfreqStepsPerRamp)
            err_SDmean[i] = np.zeros(MWfreqStepsPerRamp)
            for k in range(0, MWfreqStepsPerRamp, 1):
                SDmean[i][k] = np.mean(SD_binary[k])
                err_SDmean[i][k] = np.std(SD_binary[k]) / np.sqrt(len(SD_binary[k]))
    ...
    def dataEv_writeAtom(self, path: str, file_list : List[str], filetype : str = 'h5', position : int = 0):
        ...
        print("Efficiency herald total = %.4f(%d)"%(photonkcEff, photonkcEfferr*1e4))

        pF2, pF2err = (kcHF2 + kcVF2)/(kcHF1 + kcHF2 + kcVF1 + kcVF2), np.sqrt(kcHF2 + kcVF2)/(kcHF1 + kcHF2 + kcVF1 + kcVF2)
        pF1, pF1err = (kcHF1 + kcVF1)/(kcHF1 + kcHF2 + kcVF1 + kcVF2), np.sqrt(kcHF1 + kcVF1)/(kcHF1 + kcHF2 + kcVF1 + kcVF2)
    ...
    def channels_histo(self, dataDic: dict, detectors = ["ch2", "ch3", "ch4", "ch7"], gates = [0], binNum: int = 10000, trigger : str = "ch5",
    ...
    def g2(self, counts1 , counts2 , seqDur, filename : str = " "):
        ...
        g2err = np.clip(coincidences, 1, 1e15) / np.average(np.delete(coincidences, 10)) * np.sqrt(1 / np.clip(coincidences, 1, 1e15) + 1 / np.sum(np.delete(coincidences, 10)))
    ...
    def splitting_vs_power(self, power, splitting, splittingstd, fileName: str):
        ...
        p0 = (-12, 4, 100)
        param_bounds = ([-15, -3, 0], [15, 1e4, 1000])

        err_p = 0.01 * np.array(power)

        popt, pcov = curve_fit(curve, power, splitting, p0, bounds=param_bounds, maxfev=10000)
        print(popt)

        plt.rcParams.update({'font.size': 18})
        plt.close("all")
        fig1 = plt.figure(fileName, figsize=(11, 8))
        fig1.suptitle(
            r"Zeeman splitting $vs$ red trap power" + "\n" + r"Fit : $a \times (x - x_0) + b$" + "\n a = %.3f pm %.3f" % (
            popt[1], np.sqrt(pcov[1][1])))

        ax1 = fig1.add_subplot(1, 1, 1)

        pfit = np.linspace(0, 140, 10000)
        print(splitting)
        ax1.errorbar(power, splitting, splittingstd, err_p, linestyle="", marker="o", capsize=3, color="orange")
        ax1.plot(pfit, curve(pfit, *popt), ls="-", lw=2, color="orange")
        ax1.set_xlabel("Red Trap power (mW)")
        ax1.set_ylabel(r"$(f_{mF'=+1 \leftrightarrow mF = 0}) - (f_{mF'=-1 \leftrightarrow mF = 0}$)" + " (kHz)")
        ax1.grid()
        ax1.set_xlim(0, 140)
...
class stateTomography:
    ...
    def stokes_2q(self, coincidences : dict, prntP : bool):
        ...
        S[1][1] = p['X_DA']['F1, H']['p'] - p['X_DA']['F1, V']['p'] - p['X_DA']['F2, H']['p'] + p['X_DA']['F2, V']['p']
        S[1][2] = p['X_HV']['F1, H']['p'] - p['X_HV']['F1, V']['p'] - p['X_HV']['F2, H']['p'] + p['X_HV']['F2, V']['p']
        S[1][3] = p['X_RL']['F1, H']['p'] - p['X_RL']['F1, V']['p'] - p['X_RL']['F2, H']['p'] + p['X_RL']['F2, V']['p']
        ...
        S[3][1] = p['Z_DA']['F1, H']['p'] - p['Z_DA']['F1, V']['p'] - p['Z_DA']['F2, H']['p'] + p['Z_DA']['F2, V']['p']
        S[3][3] = p['Z_RL']['F1, H']['p'] - p['Z_RL']['F1, V']['p'] - p['Z_RL']['F2, H']['p'] + p['Z_RL']['F2, V']['p']
        S[3][2] = p['Z_HV']['F1, H']['p'] - p['Z_HV']['F1, V']['p'] - p['Z_HV']['F2, H']['p'] + p['Z_HV']['F2, V']['p']

        S[0][1] = p['X_DA']['F1, H']['p'] - p['X_DA']['F1, V']['p'] + p['X_DA']['F2, H']['p'] - p['X_DA']['F2, V']['p']
        S[0][2] = p['Y_HV']['F1, H']['p'] - p['Y_HV']['F2, H']['p'] + p['Y_HV']['F1, V']['p'] - p['Y_HV']['F2, V']['p']
        S[0][3] = p['Z_RL']['F1, H']['p'] - p['Z_RL']['F1, V']['p'] + p['Z_RL']['F2, H']['p'] - p['Z_RL']['F2, V']['p']
        S[1][0] = p['X_DA']['F1, H']['p'] + p['X_DA']['F1, V']['p'] - p['X_DA']['F2, H']['p'] - p['X_DA']['F2, V']['p']
        S[2][0] = p['Y_HV']['F1, H']['p'] + p['Y_HV']['F2, H']['p'] - p['Y_HV']['F1, V']['p'] - p['Y_HV']['F2, V']['p']
        S[3][0] = p['Z_RL']['F1, H']['p'] + p['Z_RL']['F1, V']['p'] - p['Z_RL']['F2, H']['p'] - p['Z_RL']['F2, V']['p']

        S_err[1][1] = np.sqrt(p['X_DA']['F1, H']['err']**2 + p['X_DA']['F1, V']['err']**2 + p['X_DA']['F2, H']['err']**2 + p['X_DA']['F2, V']['err']**2)
        S_err[1][2] = np.sqrt(p['X_HV']['F1, H']['err']**2 + p['X_HV']['F1, V']['err']**2 + p['X_HV']['F2, H']['err']**2 + p['X_HV']['F2, V']['err']**2)
        S_err[1][3] = np.sqrt(p['X_RL']['F1, H']['err']**2 + p['X_RL']['F1, V']['err']**2 + p['X_RL']['F2, H']['err']**2 + p['X_RL']['F2, V']['err']**2)

        S_err[2][1] = np.sqrt(p['Y_DA']['F1, H']['err']**2 + p['Y_DA']['F1, V']['err']**2 + p['Y_DA']['F2, H']['err']**2 + p['Y_DA']['F2, V']['err']**2)
        S_err[2][2] = np.sqrt(p['Y_HV']['F1, H']['err']**2 + p['Y_HV']['F1, V']['err']**2 + p['Y_HV']['F2, H']['err']**2 + p['Y_HV']['F2, V']['err']**2)
        S_err[2][3] = np.sqrt(p['Y_RL']['F1, H']['err']**2 + p['Y_RL']['F1, V']['err']**2 + p['Y_RL']['F2, H']['err']**2 + p['Y_RL']['F2, V']['err']**2)

        S_err[3][1] = np.sqrt(p['Z_DA']['F1, H']['err']**2 + p['Z_DA']['F1, V']['err']**2 + p['Z_DA']['F2, H']['err']**2 + p['Z_DA']['F2, V']['err']**2)
        S_err[3][3] = np.sqrt(p['Z_RL']['F1, H']['err']**2 + p['Z_RL']['F1, V']['err']**2 + p['Z_RL']['F2, H']['err']**2 + p['Z_RL']['F2, V']['err']**2)
        S_err[3][2] = np.sqrt(p['Z_HV']['F1, H']['err']**2 + p['Z_HV']['F1, V']['err']**2 + p['Z_HV']['F2, H']['err']**2 + p['Z_HV']['F2, V']['err']**2)

        S_err[0][1] = np.sqrt(p['X_DA']['F1, H']['err']**2 + p['X_DA']['F1, V']['err']**2 + p['X_DA']['F2, H']['err']**2 + p['X_DA']['F2, V']['err']**2)
        S_err[0][2] = np.sqrt(p['Y_HV']['F1, H']['err']**2 + p['Y_HV']['F2, H']['err']**2 + p['Y_HV']['F1, V']['err']**2 + p['Y_HV']['F2, V']['err']**2)
        S_err[0][3] = np.sqrt(p['Z_RL']['F1, H']['err']**2 + p['Z_RL']['F1, V']['err']**2 + p['Z_RL']['F2, H']['err']**2 + p['Z_RL']['F2, V']['err']**2)
        S_err[1][0] = np.sqrt(p['X_DA']['F1, H']['err']**2 + p['X_DA']['F1, V']['err']**2 + p['X_DA']['F2, H']['err']**2 + p['X_DA']['F2, V']['err']**2)
        S_err[2][0] = np.sqrt(p['Y_HV']['F1, H']['err']**2 + p['Y_HV']['F2, H']['err']**2 + p['Y_HV']['F1, V']['err']**2 + p['Y_HV']['F2, V']['err']**2)
        S_err[3][0] = np.sqrt(p['Z_RL']['F1, H']['err']**2 + p['Z_RL']['F1, V']['err']**2 + p['Z_RL']['F2, H']['err']**2 + p['Z_RL']['F2, V']['err']**2)
    ...
    def fidelity_2q(self, state, dm, error=False):
        ...
        if error:
            for i, sigma1 in enumerate(self.pauli):
                for j, sigma2 in enumerate(self.pauli):
                    f_err = f_err + 1 / 16 * self.s_err[i][j] ** 2 * np.abs(state.dag()*tensor(sigma1, sigma2)*state) ** 2
            f_err = np.sqrt(f_err)
    ...
    def bestfidelity_1q(self, state, dm, error=False):
        ...
        for i, sigma in enumerate(self.pauli):
            dm_best = dm_best + 1 / 2 * self.s[i] * self.R(res.x[0], res.x[1]) * sigma * self.R(res.x[0], res.x[1]).dag()
    ...
    def bestfidelity_2q(self, state, dm, error=False):
        ...
        def minus_fidelity(x):
            ...
            for i, sigma1 in enumerate(self.pauli):
                for j, sigma2 in enumerate(self.pauli):
                    dm_r = dm_r + 1 / 4 * self.s[i][j] * tensor(self.R(x[0], x[1]) * sigma1 * self.R(x[0], x[1]).dag(),self.R(x[2], x[3]) * sigma2 * self.R(x[2], x[3]).dag())
        ...
        if error:
            for i, sigma1 in enumerate(self.pauli):
                for j, sigma2 in enumerate(self.pauli):
                    f_err = f_err + 1 / 16 * self.s_err[i][j] ** 2 * np.abs(state.dag()*tensor(self.R(res.x[0], res.x[1]) * sigma1 * self.R(res.x[0], res.x[1]).dag(),self.R(res.x[2], res.x[3]) * sigma2 * self.R(res.x[2], res.x[3]).dag())*state) ** 2

            f_err = np.sqrt(f_err)

        for i, sigma1 in enumerate(self.pauli):
            for j, sigma2 in enumerate(self.pauli):
                dm_best = dm_best + 1 / 4 * self.s[i][j] * tensor(self.R(res.x[0], res.x[1]) * sigma1 * self.R(res.x[0], res.x[1]).dag(), self.R(res.x[2],res.x[3]) * sigma2 * self.R(res.x[2], res.x[3]).dag())

        return f_best, f_err, dm_best

if __name__ == '__main__':
    """
    This script analyzes a Microwave Spectrum.
    """
    ...
    measurements = ("26_03_26_VTrap_calibration_4")
    filetype : str = ".h5"
    # fileSave : str = "coincidences_" + "19_11_24_tomography_3" + "_kcH"

    # ------ Begin Analysis ------
    analysis = AtomAnalysis()
    # ------ Data evaluation ------
    trapPws = np.array([1, 0.75, .5, .4, .3]) * 130
    # trapPws = np.array([1, 0.75, .5, .4]) * 130
    fig = [0 for pw in trapPws]
    ax = [0 for pw in trapPws]
    delta = [0 for pw in trapPws]
    delta_stderr = [0 for pw in trapPws]
    Delta = [0 for pw in trapPws]
    Delta_stderr = [0 for pw in trapPws]

    (MWfrequencies, SDmean, err_SDmean) = analysis.dataEv_MWspectrumTrapScan(path, measurements, filetype, trapPws)

    MWfreqCenter = 0
    MWfreqStart, MWfreqEnd = MWfrequencies[0], MWfrequencies[-1]
    MWfreqStepsPerRamp = len(MWfrequencies)
    MWfreqStep = 1 #kHz

    for i in range(len(trapPws)):
        ...
        if fit_peaks:
            ...
            threshold = 0.15
            window_size = int(MWfreqStepsPerRamp / 2 - 1)
            maxima_indices, maxima_values = analysis.find_local_maxima_in_range(np.array(SDmean[i]), window_size, threshold)

            print("Indices of local maxima above threshold:", maxima_indices)
            print("Local maxima values above threshold:", maxima_values)
            print("Frequencies of local maxima values above threshold:", [MWfrequencies[j] for j in maxima_indices])

            # # fitting the two different peaks
            modMWspec = [None] * len(maxima_values)
            res_MWspec = [None] * len(maxima_values)

            c0, c1, l0, l1, r0, r1 = 0, 0, 0, 0, 0, 0  # center0, center1, center2

            if n_peaks == 2:

                c = np.array([MWfrequencies[maxima_index] for maxima_index in maxima_indices])

                l = c - 5
                r = c + 5

                l_index = (maxima_indices - 8/MWfreqStep).astype(int)
                r_index = (maxima_indices + 8/MWfreqStep).astype(int)
                ...
                for j in range(len(modMWspec)):
                    print("index = ", j)
                    print(l[j], r[j])
                    print(MWfrequencies[l_index[j]:r_index[j]])
                    print(l_index)
                    print(r_index)
                    print("--------------------")
                    print(
                        MWfrequencies[l_index[j]:r_index[j]],
                        SDmean[i][l_index[j]:r_index[j]],
                        err_SDmean[i][l_index[j]:r_index[j]]
                    )
                    print("--------------------")
                    (modMWspec[j], res_MWspec[j]) = analysis.dataFit_lorenzian(MWfrequencies[l_index[j]:r_index[j]],
                                                                               SDmean[i][l_index[j]:r_index[j]],
                                                                               err_SDmean[i][l_index[j]:r_index[j]])
                try:
                    freqFitPlot = [np.linspace(MWfrequencies[l_index[j]], MWfrequencies[r_index[j]], 1000) for j in
                                   range(len(modMWspec))]
                except:
                    freqFitPlot = [np.linspace(MWfrequencies[l_index[j]], MWfrequencies[-1], 1000) for j in
                                   range(len(modMWspec))]

        # ------ Plotting ------
        afs = 14  # axis font size
        txt = measurements + " - trap Power = %.1f mW"%(trapPws[i])
        fig = plt.figure(txt, figsize=(11, 5.5))
        fig.suptitle(txt)
        ax1 = fig.add_subplot(1, 1, 1)

        plt.errorbar(MWfrequencies, SDmean[i], yerr=err_SDmean[i], marker = "o", linestyle = "None",color="black", capsize=3)
        string = measurements + '\n'
        if fit_peaks:
            for j in range(len(modMWspec)):
                ax1.plot(freqFitPlot[j], modMWspec[j].eval(freq=freqFitPlot[j], params=res_MWspec[j].params), "r")
                Delta[i] = np.round(res_MWspec[1].best_values['foff'], 1) - np.round(res_MWspec[0].best_values['foff'],
                                                                                     1)
                Delta_stderr[i] = np.round(res_MWspec[1].params['foff'].stderr, 2) + np.round(res_MWspec[0].params['foff'].stderr, 2)

                fig.suptitle(measurements + " - trap Power = %.1f mW"%(trapPws[i]) + '\n' +
                             'foffs1 = ' + str(
                    np.round(res_MWspec[0].best_values['foff'], 1)) + 'kHz' + '       FWHM1 = ' + str(
                    np.round(2 * res_MWspec[0].best_values['k1'], 2)) + 'kHz\n'
                                                                        'foffs2 = ' + str(
                    np.round(res_MWspec[1].best_values['foff'], 1)) + 'kHz' + '       FWHM2 = ' + str(
                    np.round(2 * res_MWspec[1].best_values['k1'], 2)) + 'kHz\n'
                                                                        'foffs3 = ' + str(Delta[i]) + ' pm ' + str(Delta_stderr[i]) + ' kHz', fontsize=afs)

        else:
            fig.suptitle(string, fontsize=afs)

        MWfreqCenter = 6804678 + 30000  # in kHz
        ax1.set_xlabel(r"MW frequency $\nu - $%.6f GHz (kHz)" % (MWfreqCenter * 1e-6), fontsize=afs)
        ax1.set_ylabel("Population in F=2", fontsize=afs)
        ax1.tick_params(labelsize=afs - 1, direction='in', top=False, right=False)
    ...
    fig_final = analysis.splitting_vs_power(trapPws[:], Delta[:], Delta_stderr[:], measurements)

    fig_final.savefig(path + "\\" + fig_final.get_label() + ".pdf")
    fig_final.savefig(path + "\\" + fig_final.get_label() + ".png")
