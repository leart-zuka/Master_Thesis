...
# Time window to display (seconds from start of each atom)
t_start = 0.0
t_end = 3.0  # adjust to capture your loading algorithm
...
for atom_idx in range(len(data) - 1):
    ...
    # Slice to the desired time window
    ...
    ax.set_title(
        f"Atom {atom_idx} — loading gap: {gap:.2f} s  |  showing [{t_start}, {t_end}] s"
    )
    ax.legend()
    plt.tight_layout()
    plt.show()
