...
class AtomAnalysis:
    ...
    def dataEv_postSelection(
        ...
        for i in tqdm(atomList, file=sys.stdout):
            ...
            wt_kc = 0.8 * kcCounts  # wt = witness threshold
            wt_lc = -1  # wt = witness threshold
            twot = 2.4 * kcCounts  # twot = two atom threshold
            inAtom = False
            atomIn_index = 0
            atomOut_index = 0
    ...
    def dataEval_noramlModeSpectroscopy(
        ...
        for filename in file_list:
            ...
            if not os.path.exists(file_postSelected):
                self.dataEv_postSelection(
                    path, filename, filetype, kcCounts=1500, no=10
                )

            a_file = open(file_postSelected, "rb")
            atomDic = pickle.load(a_file)
            # atomDic = {2: [1.1041563265534933, 4.934601188728266],
            #            4: [0.6540761673168163, 61.718255621352]}
            ...
            # chfig = self.channels_histo(dataDic, detectors, gates, binNum, trig, maxTrigDiff, fsdelay, filename,colors)
            # plt.show(block=True)
            ...
            print("""
                ...
                """)
            # print(dataDic)
            # print(len(dataDic[self.syncFast][1]))
            # print(len(dataDic[scanTrig][1]))
        ...
        for point in range(0, int(ParamDic["PointsPerScan"] / 2), 1):
            # list_SDmean[0][point] = np.mean(R_binary_up[point])
            # list_err_SDmean[0][point] = np.std(R_binary_up[point]) / np.sqrt(len(R_binary_up[point]))
            #
            # list_SDmean[1][point] = np.mean(R_binary_down[point])
            # list_err_SDmean[1][point] = np.std(R_binary_down[point]) / np.sqrt(len(R_binary_down[point]))
        ...
        # list_SDmean[2] = list_SDmean[2] + np.flip(list_SDmean[3])
        # list_err_SDmean[2] = list_err_SDmean[2] + np.flip(list_err_SDmean[3])
        ...
        def R_coupled_star(detuning, g, f_res, A, offset, a):
            ...
            r = Mu_fr - Mu_fc**2 * (2 * Kappa_oc) / (Kappa + 1j * Delta) * 1 / (
                2 * C_d + 1
            )
            return A * np.abs(r) ** 2 + offset

        def R_coupled(
            detuning, f_res, A, g, kappa, kappa_oc, MM_rf, MM_fc, gamma, offset, a
        ):
        ...
        # ]
        # bounds = (
        #     [-50, 0.01, 30, 55.4, 55.4 * 0.8, 0.95, 0.7, 2.9, 0.01, -10],
        #     [-20, 0.3, 85, 60, 51, 1, 0.9, 3.3, 0.03, 10],
        # )  # Avoid zero/negative sigma/lambda
        # popt, pcov = curve_fit(
        #     R_coupled, freq_NMS, list_SDmean[0], p0=p0, bounds=bounds
        # )

        p0_r = [30, -15, 0.2, 0.2, 0.01]
        ...
        df.to_csv("nms_errorbar_data.csv", index=False)
...
if __name__ == "__main__":
    ...
    ParamDic = {
        "freqSpan": 275,  # in MHz
        "PointsPerScan": 400,  # including up and down ramp
        "TrialsPerPoint": 40,
        "freqCenter": 162.5,
    }

    analysis.dataEval_noramlModeSpectroscopy(
        path, file_list, filetype, position=0, ParamDic=ParamDic
    )
