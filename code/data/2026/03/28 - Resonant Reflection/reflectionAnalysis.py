...
class Analyzer:
    ...
    def reflection_analysis(
        ...
        P_4 = tot_clicks_4 / tot_clicks
        P_7 = tot_clicks_7 / tot_clicks

        P_4_err = np.sqrt(P_4 * (1 - P_4) / tot_clicks)
        P_7_err = np.sqrt(P_7 * (1 - P_7) / tot_clicks)

        self.data_arr = None

        return P_4, P_4_err, P_7, P_7_err
...
if __name__ == "__main__":
    ...
    file = "28_03_26_Resonant_Reflection_R_005_photons_atom_1_trap_on_test_basis_1"
    ...
    # P_4, P_4_err, P_7, P_7_err = analyzer.reflection_analysis(file_name=file, path="./",
    #                                                           parameters=ParamDictReflection_trap_off,
    #                                                           plot_histogram=False, post_select_sd=True)
    # print(f"Prob to land in ch4: {P_4 * 100:.2f}+/-{P_4_err * 100:.2f}")
    # print(f"Prob to land in ch7: {P_7 * 100:.2f}+/-{P_7_err * 100:.2f}")
    #
    ...
    for i, file in enumerate(cnot_files_trap_off):
        P_4, P_4_err, P_7, P_7_err = analyzer.reflection_analysis(
            file_name=file,
            path="./",
            parameters=ParamDictReflection_trap_off,
        ...
        if i < 2:
            cnot_gate[0][i] = P_4
            cnot_err[0][i] = P_4_err
            cnot_gate[1][i] = P_7
            cnot_err[1][i] = P_7_err
        else:
            cnot_gate[2][i] = P_4
            cnot_err[2][i] = P_7_err
            cnot_gate[3][i] = P_7
            cnot_err[3][i] = P_7_err
    ...
    plot_gate_3d(
        cnot_gate,
        cnot_err,
        title=r"CNOT Gate (R/L basis)",
        file_name="CNOT @ mean photon of 1_6 RL",
