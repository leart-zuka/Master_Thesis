...
At the heart of the CavityX system are the two crossed fiber cavities which are named the "short" cavity (SC) and the "long" cavity (LC), which differ in length ($\SI{79}{\mu\metre}$ vs. $\SI{161}{\mu\metre}$), and in the fact that the short cavity is birefringent. Each cavity fiber is held in place by an in-house built holder shown in figure \ref{fig:cavities-setup}.
...
The cavity fibers are fabricated using a $\text{CO}_2$ laser machining technique by Manuel Brekenfeld in \cite{ManuelBThesis2023}. Changing the beam profile of the $\text{CO}_2$ laser allows for the fabrication of either spherical or elliptical surfaces, as is the case for the short cavity.\\
...
\begin{table}[ht]
	...
	\begin{tabular}{l|c|c}
		...
		Mode matching                                     &                         & \notetome{write down actual $\mu_{fc}$ here} \\
		$g(1' \leftrightarrow 2)/(2\pi)$ at cavity center &                         & \notetome{write down actual g here}          \\
		...
		$\kappa/(2\pi)$                                   &                         & \notetome{write down $\kappa$ here}          \\
		Linewidth                                         &                         & \notetome{write down LW here}                \\
		Finesse                                           & $14089$                 & $16531$                                      \\
		Outcoupling ratio                                 &                         & \notetome{write down outcoupling ratio here} \\
		Eigenmodes freq.\ splitting                       & $16\,$MHz               & $505.0(5)\,$MHz                              \\
		\hline\hline
	\end{tabular}
	\caption{\notetome{write down table title and also notes regarding the subscripts in there}}
\end{table}
...
A different atom-positioning algorithm was employed in the past (e.g. the one used in Gianvito Chiarella's thesis \cite{GianvitoC2026}), where the FPGA only scanned up to the point where the maximum fluorescence counts were reached. This past algorithm could load two atoms which were badly coupled to the cavity since a doubly-loaded node was indistinguishable from a well-coupled single atom. By completing the scan over the entire fluorescence counts curve, one is able to properly ensure single atoms are being trapped.
...
\section{Atomic Qubit Preparation}
...
By scanning the length of the $\pi$ pulse at a constant MW frequency, one can determine the ideal time it takes for a state to coherently transfer from $F=1,m_F=0$ to $F=2,m_F=0$. Figure \ref{fig: rabi flop} shows the resulting Rabi oscillations for an atom prepared in $F=1,m_F=0$ and transferred to the $F=2,m_F=0$ state. Fitting equation \eqref{eq:rabi_oscillation_model}: \notetome{write down fit params}
